    ...
    def _promote_visible_answer_from_submit_evidence(
        ...
        *,
        ...
        if not isinstance(submit_evidence, dict):
            return None
        # v0.0.278.60: never promote a visible JSON-looking answer after an
        # unconfirmed submit.  Promotion is only safe when the current prompt
        # has a proven submit and a visible post-submit user turn; otherwise old
        # JSON smoke answers can be mistaken for the current answer.
        if not bool(submit_evidence.get("submit_confirmed")):
            return None
        if submit_evidence.get("post_submit_user_turn_visibility_checked") and not bool(submit_evidence.get("post_submit_user_turn_visible")):
            return None
