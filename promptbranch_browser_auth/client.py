...
class ChatGPTBrowserClient:
    ...
    async def _ask_question_operation(
        ...
        *,
        ...
        phase_timings["submit_prepare_request_count"] = submit_evidence.get("submit_prepare_request_count")
        phase_timings["submit_prepare_only"] = submit_evidence.get("submit_prepare_only")
        phase_timings["submit_prepare_first_observed_after_click_seconds"] = submit_evidence.get("submit_prepare_first_observed_after_click_seconds")
        phase_timings["submit_message_request_observed"] = submit_evidence.get("submit_message_request_observed")
        phase_timings["submit_message_request_count"] = submit_evidence.get("submit_message_request_count")
        ...
        phase_timings["submit_prepare_conduit_token_latest_sha256_12"] = submit_evidence.get("submit_prepare_conduit_token_latest_sha256_12")
        phase_timings["submit_prepare_conduit_token_sha256_12_values"] = submit_evidence.get("submit_prepare_conduit_token_sha256_12_values")
        phase_timings["submit_prepare_conduit_token_count"] = submit_evidence.get("submit_prepare_conduit_token_count")
        phase_timings["submit_prepare_conduit_token_active_policy"] = submit_evidence.get("submit_prepare_conduit_token_active_policy")
        ...
        if not bool(submit_evidence.get("submit_confirmed")):
            ...
            failure_error = (
                ...
                if failure_reason == "stream_started_without_user_message_commit"
    ...
    def _submit_prepare_commit_timeout_ms(self) -> int:
        raw = (os.getenv("CHATGPT_SUBMIT_PREPARE_COMMIT_TIMEOUT_MS") or "3000").strip()
        try:
            return max(250, min(15_000, int(raw)))
        except ValueError:
            return 3_000

    def _submit_prepare_fast_fail_enabled(self) -> bool:
        value = (os.getenv("CHATGPT_SUBMIT_PREPARE_FAST_FAIL") or "1").strip().lower()
        return value not in {"0", "false", "no", "off", "disabled"}

    def _submit_prepare_fast_fail_timeout_ms(self) -> int:
        raw = (os.getenv("CHATGPT_SUBMIT_PREPARE_FAST_FAIL_TIMEOUT_MS") or "1500").strip()
        try:
            return max(250, min(10_000, int(raw)))
        except ValueError:
            return 1_500
    ...
    def _find_conduit_token_values(self, value: Any, *, max_depth: int = 6, limit: int = 20) -> list[str]:
        """Return raw conduit tokens for private in-memory tracing only.

        Callers must never put returned values into structured output or logs.
        Public diagnostics expose only sha256_12 fingerprints and counts.
        """

        found: list[str] = []

        def add_token(candidate: Any) -> None:
            token_text = str(candidate or "")
            if token_text and token_text not in found and len(found) < limit:
    ...
    def _find_conduit_token_value(self, value: Any, *, max_depth: int = 6) -> str | None:
        values = self._find_conduit_token_values(value, max_depth=max_depth, limit=1)
        return values[0] if values else None

    def _extract_conduit_tokens_from_text(self, text: str) -> list[str]:
        body_text = text or ""
        if not body_text:
            return []
        try:
            ...
            return []
        return self._find_conduit_token_values(parsed)

    def _extract_conduit_token_from_text(self, text: str) -> str | None:
        tokens = self._extract_conduit_tokens_from_text(text)
    ...
    def _private_conduit_token_hashes_in_text(self, text: Any, tokens: list[str]) -> list[str]:
        body = str(text or "")
    ...
    def _text_contains_private_conduit_token(self, text: Any, tokens: list[str]) -> bool:
        return bool(self._private_conduit_token_hashes_in_text(text, tokens))
    ...
    def _register_private_conduit_token(self, observer: Any, token: str) -> None:
        ...
        if sha and not observer.get("prepare_conduit_token_sha256_12"):
            observer["prepare_conduit_token_sha256_12"] = sha
        if sha:
            observer["prepare_conduit_token_latest_sha256_12"] = sha
        observer["prepare_conduit_token_count"] = len(tokens)
        observer["prepare_conduit_token_active_policy"] = "all_tokens"
    ...
    def _record_submit_websocket_frame(self, observer: Any, socket_record: dict[str, Any], *, direction: str, payload: Any) -> None:
        ...
        matched_hashes = self._private_conduit_token_hashes_in_text(text, tokens)
        if matched_hashes:
            frame["conduit_token_seen"] = True
            frame["conduit_token_sha256_12"] = matched_hashes[0]
            frame["conduit_token_sha256_12_values"] = matched_hashes[:10]
    ...
    def _submit_response_body_shape(self, text: str, *, url: str = "", status: Any = None) -> dict[str, Any]:
        ...
        shape: dict[str, Any] = {
            ...
            "conduit_token_sha256_12_values": [],
            "conduit_token_count": 0,
        }
        if not body_text:
            return shape
        try:
        ...
        conduit_tokens = self._find_conduit_token_values(parsed)
        conduit_hashes = [
            self._submit_conduit_token_sha256_12(token)
            for token in conduit_tokens
            if self._submit_conduit_token_sha256_12(token)
        ]
        shape["conduit_token_present"] = bool(conduit_tokens)
        shape["conduit_token_sha256_12"] = conduit_hashes[0] if conduit_hashes else None
        shape["conduit_token_sha256_12_values"] = conduit_hashes[:10]
        shape["conduit_token_count"] = len(conduit_tokens)
        return shape
    ...
    async def _capture_submit_response_body_shape(self, response: Any, record: dict[str, Any], observer: Any | None = None) -> None:
        ...
        try:
            ...
            body_tokens = self._extract_conduit_tokens_from_text(text)
            if body_tokens and record.get("summary_kind") == "prepare":
                for token in body_tokens:
                    self._register_private_conduit_token(observer, token)
            tokens = self._conduit_tokens_private(observer)
            matched_hashes = self._private_conduit_token_hashes_in_text(text, tokens)
            if matched_hashes and record.get("summary_kind") != "prepare":
                record["conduit_token_seen_in_response"] = True
                record["conduit_token_sha256_12"] = matched_hashes[0]
                record["conduit_token_sha256_12_values"] = matched_hashes[:10]
    ...
    def _submit_request_resource_type(self, request: Any) -> str | None:
        try:
    ...
    def _submit_request_initiator_summary(self, request: Any) -> dict[str, Any]:
        ...
        try:
            ...
            if callable(initiator):
                initiator = initiator()
        except Exception:
            initiator = None
        if isinstance(initiator, dict):
            summary["available"] = True
            summary["source"] = "request.initiator"
            summary["type"] = str(initiator.get("type") or "")[:80] or None
            summary["url"] = str(initiator.get("url") or "")[:240] or None
            summary["line_number"] = initiator.get("lineNumber") or initiator.get("line_number")
            summary["column_number"] = initiator.get("columnNumber") or initiator.get("column_number")
            return summary
        try:
        ...
        if frame_url:
            summary["available"] = True
            summary["source"] = "request.frame"
            summary["type"] = "frame"
            summary["url"] = str(frame_url)[:240]
        return summary

    def _submit_request_navigation_flag(self, request: Any) -> bool | None:
        try:
            value = getattr(request, "is_navigation_request", None)
            if callable(value):
                return bool(value())
    ...
    def _submit_request_redirect_summary(self, request: Any) -> dict[str, Any]:
        summary: dict[str, Any] = {"redirected_from_url": None, "redirected_to_url": None}
        for attr_name, key in (("redirected_from", "redirected_from_url"), ("redirected_to", "redirected_to_url")):
            try:
                redirected = getattr(request, attr_name, None)
                if callable(redirected):
                    redirected = redirected()
                url = getattr(redirected, "url", None) if redirected is not None else None
                if callable(url):
                    url = url()
                if url:
                    summary[key] = str(url)[:240]
            except Exception:
                pass
        return summary

    def _submit_network_record_is_message_submit(self, record: Any) -> bool:
        """Return True only for a marker-bearing final message-submit request.

        `/backend-api/f/conversation/prepare` is diagnostic prepare traffic.  It may
        carry the current prompt marker, but it must not satisfy submit causality;
        otherwise the retry path is skipped after a prepare-only request.
        """
        if not isinstance(record, dict):
            return False
        if not bool(record.get("marker_found")):
            return False
        try:
            url = str(record.get("url") or "")
        except Exception:
        ...
        backend_like = bool(record.get("backend_like")) or ("chatgpt.com" in host and "/backend-api/" in path)
        mutating = bool(record.get("mutating")) or method in {"POST", "PUT", "PATCH"}
        if bool(record.get("prepare_request")) or path.endswith("/backend-api/f/conversation/prepare"):
            return False
        if record.get("message_request_candidate") is False:
            return False
        return bool(backend_like and mutating)
    ...
    def _submit_network_request_record(self, request: Any, *, prompt: str | None) -> dict[str, Any]:
        ...
        resource_type = self._submit_request_resource_type(request)
        initiator = self._submit_request_initiator_summary(request)
        navigation_request = self._submit_request_navigation_flag(request)
        redirects = self._submit_request_redirect_summary(request)
    ...
    def _start_submit_network_observer(self, page: Any, *, prompt: str | None) -> dict[str, Any]:
        ...
        def on_request(request: Any) -> None:
            ...
            try:
                ...
                tokens = self._conduit_tokens_private(observer)
                matched_hashes = self._private_conduit_token_hashes_in_text(record.get("_private_combined_text"), tokens)
                if matched_hashes:
                ...
                if (
                    record.get("marker_found")
                    and self._submit_network_record_is_message_submit(record)
                    and observer.get("matched_request") is None
                ):
                ...
                elif record.get("marker_found") and record.get("prepare_request"):
                    self._log(
                        "submit",
                        "prepare request carrying current marker observed but rejected as submit confirmation",
                        url=record.get("url"),
                        method=record.get("method"),
                        matched_by=record.get("matched_by"),
                        matched_marker=record.get("matched_marker"),
        ...
        def on_response(response: Any) -> None:
            ...
            try:
                ...
                request_record = self._submit_network_request_record(request, prompt=prompt) if request is not None else {}
                method = str(request_record.get("method") or getattr(request, "method", "") or "").upper() if request is not None else ""
                record = {
                    "url": url,
                    "method": method,
                    "status": status,
                    "backend_like": bool(request_record.get("backend_like")) or "/backend-api/" in url or "/conversation" in url,
                    "mutating": bool(request_record.get("mutating")),
                    "prepare_request": bool(request_record.get("prepare_request")),
                    "message_request_candidate": bool(request_record.get("message_request_candidate")),
                    "marker_found": bool(request_record.get("marker_found")),
                    "post_data_length": request_record.get("post_data_length"),
                    "resource_type": request_record.get("resource_type"),
                    "initiator": request_record.get("initiator"),
                    "navigation_request": request_record.get("navigation_request"),
                    "redirected_from_url": request_record.get("redirected_from_url"),
                    "redirected_to_url": request_record.get("redirected_to_url"),
                ...
                elif len(observer["responses"]) < 40 and (record.get("backend_like") or record.get("prepare_request") or record.get("message_request_candidate")):
    ...
    def _submit_network_evidence_snapshot(self, observer: Any) -> dict[str, Any]:
        ...
        all_events = [event for event in (observer.get("all_events") or events or []) if isinstance(event, dict)]
        responses = [event for event in (observer.get("responses") or []) if isinstance(event, dict)]
        all_responses = [event for event in (observer.get("all_responses") or responses or []) if isinstance(event, dict)]
        matched = observer.get("matched_request") if isinstance(observer.get("matched_request"), dict) else None
        if matched is not None and not self._submit_network_record_is_message_submit(matched):
            matched = None
        matched_response = observer.get("matched_response") if isinstance(observer.get("matched_response"), dict) else None
        found = matched is not None and self._submit_network_record_is_message_submit(matched)
        ...
        prepare_response_events = [event for event in all_responses if event.get("prepare_request")]
        message_response_events = [event for event in all_responses if event.get("message_request_candidate")]
        prepare_only = bool(prepare_events and not message_events and not found)
        if found:
        ...
        observer_started = observer.get("started_at_monotonic")
        prepare_first_after_click = None
        if prepare_events and isinstance(observer_started, (int, float)):
            prepare_captured = [
                event.get("captured_at_monotonic")
                for event in prepare_events
                if isinstance(event.get("captured_at_monotonic"), (int, float))
            ]
            if prepare_captured:
                prepare_first_after_click = round(min(prepare_captured) - float(observer_started), 3)
        prepare_first_captured_at = None
        if prepare_events:
            captured_values = [
                event.get("captured_at_monotonic")
                for event in prepare_events
            ...
            if captured_values:
                prepare_first_captured_at = min(float(value) for value in captured_values)
        post_prepare_events = []
        post_prepare_responses = []
        if prepare_first_captured_at is not None:
            ...
            post_prepare_responses = [
                event for event in all_responses
                if isinstance(event.get("captured_at_monotonic"), (int, float))
                and float(event.get("captured_at_monotonic")) >= prepare_first_captured_at
            ]
        post_prepare_observation_seconds = None
        if prepare_first_captured_at is not None:
            post_prepare_observation_seconds = round(time.monotonic() - prepare_first_captured_at, 3)
        prepare_response_statuses = [event.get("status") for event in prepare_response_events[:20]]
        prepare_response_error_hint = None
        for status_code in prepare_response_statuses:
        ...
        post_prepare_stream_status_events = [
            event for event in stream_status_events
            if prepare_first_captured_at is not None
            and isinstance(event.get("captured_at_monotonic"), (int, float))
            and float(event.get("captured_at_monotonic")) >= prepare_first_captured_at
        ]
        post_prepare_stream_status_shapes = [
            event.get("body_shape")
            for event in post_prepare_stream_status_events
            if isinstance(event.get("body_shape"), dict)
        ]
        stream_status_values = [
            str(shape.get("status_value") or "")
            for shape in stream_status_shapes
            if isinstance(shape, dict) and shape.get("status_value") not in (None, "")
        ]
        post_prepare_stream_status_values = [
            str(shape.get("status_value") or "")
            for shape in post_prepare_stream_status_shapes
            if isinstance(shape, dict) and shape.get("status_value") not in (None, "")
        ]
        ...
        if not conduit_hashes:
            for shape in prepare_response_shapes:
                if not isinstance(shape, dict):
                    continue
                for value in (shape.get("conduit_token_sha256_12_values") or []):
                    value_text = str(value or "")
                    if value_text and value_text not in conduit_hashes:
                        conduit_hashes.append(value_text)
                if shape.get("conduit_token_sha256_12"):
                    value_text = str(shape.get("conduit_token_sha256_12") or "")
                    if value_text and value_text not in conduit_hashes:
                        conduit_hashes.append(value_text)
        primary_conduit_hash = (conduit_hashes or [None])[0]
        latest_conduit_hash = (conduit_hashes or [None])[-1]
        ...
        stream_started_without_commit = bool(
            not found
            and prepare_only
            and post_prepare_stream_status_streaming_observed
            and not message_events
        )
        if stream_started_without_commit:
            conduit_error_hint = "stream_started_without_user_message_commit"
        ...
        event_summaries = [
            {
                ...
                "navigation_request": event.get("navigation_request"),
            }
        ...
        return {
            ...
            "all_response_event_count": len(all_responses),
            ...
            "prepare_request_count": len(prepare_events),
            "prepare_only": prepare_only,
            "prepare_first_observed_after_click_seconds": prepare_first_after_click,
            "prepare_request_urls": [event.get("url") for event in prepare_events[:20]],
            "prepare_request_post_data_lengths": [event.get("post_data_length") for event in prepare_events[:20]],
            "prepare_response_observed": bool(prepare_response_events),
            "prepare_response_count": len(prepare_response_events),
            ...
            "prepare_conduit_token_latest_sha256_12": latest_conduit_hash,
            "prepare_conduit_token_sha256_12_values": conduit_hashes[:10],
            "prepare_conduit_token_count": len(conduit_hashes),
            "prepare_conduit_token_active_policy": "all_tokens" if conduit_hashes else None,
            ...
            "post_prepare_request_resource_types": [event.get("resource_type") for event in post_prepare_events[:30]],
            "post_prepare_request_initiators": [event.get("initiator") for event in post_prepare_events[:20]],
            "post_prepare_response_count": len(post_prepare_responses),
            "post_prepare_response_urls": [event.get("url") for event in post_prepare_responses[:30]],
            "post_prepare_response_resource_types": [event.get("resource_type") for event in post_prepare_responses[:30]],
            "post_prepare_response_initiators": [event.get("initiator") for event in post_prepare_responses[:20]],
            "post_prepare_stream_observed": bool(message_response_events or matched_response is not None or post_prepare_stream_status_streaming_observed),
            "message_request_observed": bool(message_events),
            "message_request_count": len(message_events),
            "message_request_urls": [event.get("url") for event in message_events[:20]],
    ...
    async def _wait_for_submit_network_evidence(
        ...
        *,
        ...
        prepare_only_fast_fail_ms: int | None = None,
    ) -> dict[str, Any]:
        started = time.monotonic()
        ...
        while True:
            ...
            if snapshot.get("visible"):
                snapshot["probe_seconds"] = round(time.monotonic() - started, 3)
                return snapshot
            if prepare_only_fast_fail_ms is not None:
                elapsed_ms = int(max(0.0, (time.monotonic() - started) * 1000))
                prepare_only_fast_fail_ready = bool(
                    snapshot.get("prepare_token_set_not_consumed")
                    and snapshot.get("prepare_conduit_token_present")
                    and not snapshot.get("message_request_observed")
                    and not snapshot.get("request_marker_found")
                    and elapsed_ms >= int(prepare_only_fast_fail_ms)
                )
                if prepare_only_fast_fail_ready:
                    snapshot["status"] = "prepare_token_set_not_consumed"
                    snapshot["prepare_only_fast_fail_used"] = True
                    snapshot["prepare_only_fast_fail_timeout_ms"] = int(prepare_only_fast_fail_ms)
                    snapshot["probe_seconds"] = round(time.monotonic() - started, 3)
                    return snapshot
            remaining = deadline - asyncio.get_running_loop().time()
            if remaining <= 0:
                if snapshot.get("prepare_request_observed") and not snapshot.get("message_request_observed") and not snapshot.get("request_marker_found"):
                    snapshot["status"] = "submit_prepare_without_message_commit"
                elif snapshot.get("backend_write_event_count"):
    ...
    async def _wait_for_backend_task_message_echo_after_prepare(
        ...
        *,
        ...
        started = time.monotonic()
        timeout_ms = self._submit_prepare_commit_timeout_ms() if timeout_ms is None else timeout_ms
        deadline = asyncio.get_running_loop().time() + (timeout_ms / 1000)
        attempt_count = 0
    ...
    async def _wait_for_submit_confirmation(
        ...
        *,
        ...
        prepare_only_fast_fail: bool = False,
    ) -> dict[str, Any]:
        ...
        if network_required:
            attempt += 1
            try:
                network_evidence = await self._wait_for_submit_network_evidence(
                    page,
                    submit_network_observer,
                    prepare_only_fast_fail_ms=self._submit_prepare_fast_fail_timeout_ms() if prepare_only_fast_fail else None,
                )
            except TypeError:
            ...
            prepare_only_initial = bool(
                network_evidence.get("prepare_request_observed")
                and not network_evidence.get("message_request_observed")
                and not network_evidence.get("request_marker_found")
            )
            if prepare_only_initial and prepare_only_fast_fail:
                ...
                if network_evidence.get("request_marker_found"):
                    network_found = True
                else:
                    network_evidence["prepare_only_fast_fail_used"] = True
                    network_evidence["prepare_only_fast_fail_timeout_ms"] = self._submit_prepare_fast_fail_timeout_ms()
                    last_network_evidence = network_evidence
                    last_backend_echo = {
                        "visible": False,
                        "status": "backend_commit_probe_skipped_due_to_prepare_fast_fail",
                        "post_prepare_commit_window_used": False,
                        "post_prepare_commit_found": False,
                        "post_prepare_commit_status": "backend_commit_probe_skipped_due_to_prepare_fast_fail",
                        "post_prepare_commit_attempt_count": 0,
                        "post_prepare_commit_seconds": 0.0,
                        "post_prepare_commit_timeout_ms": 0,
                        "backend_stale_user_turn_prefix_match_rejected": None,
                        "marker_present_in_matched_user_text": False,
                        "matched_marker": None,
                    }
                    last_post_prepare_ui_error = {
                        "visible": False,
                        "status": "post_prepare_ui_error_skipped_due_to_prepare_fast_fail",
                        "probe_seconds": 0.0,
                    }
                    attempt += 1
                    attempts.append({
                        "attempt": attempt,
                        "mode": "prepare_only_fast_fail",
                        "confirmed": False,
                        "confirmed_by": [],
                        "network_status_after_prepare_window": network_evidence.get("status"),
                        "prepare_only_fast_fail_used": True,
                        "prepare_only_fast_fail_timeout_ms": self._submit_prepare_fast_fail_timeout_ms(),
                        "backend_task_message_status": last_backend_echo.get("status"),
                        "post_prepare_ui_error_status": last_post_prepare_ui_error.get("status"),
                        "probe_seconds": network_evidence.get("probe_seconds"),
                    })
            elif prepare_only_initial:
                ...
                attempts.append({
                    "attempt": attempt,
                    "mode": "backend_commit_after_prepare",
                    "confirmed": backend_found,
                ...
                if backend_found:
                    ...
                    fields = self._submit_confirmation_timing_fields(
                        ...
                        causal_reason="backend_commit_after_prepare",
                        url_only_rejected=False,
            ...
            fields["prepare_request_observed"] = bool(network_evidence.get("prepare_request_observed"))
            fields["prepare_request_count"] = network_evidence.get("prepare_request_count")
            fields["prepare_only"] = prepare_only
            fields["prepare_first_observed_after_click_seconds"] = network_evidence.get("prepare_first_observed_after_click_seconds")
            fields["message_request_observed"] = bool(network_evidence.get("message_request_observed"))
            fields["message_request_count"] = network_evidence.get("message_request_count")
            ...
            fields["prepare_conduit_token_latest_sha256_12"] = network_evidence.get("prepare_conduit_token_latest_sha256_12")
            fields["prepare_conduit_token_sha256_12_values"] = network_evidence.get("prepare_conduit_token_sha256_12_values")
            fields["prepare_conduit_token_count"] = network_evidence.get("prepare_conduit_token_count")
            fields["prepare_conduit_token_active_policy"] = network_evidence.get("prepare_conduit_token_active_policy")
            ...
            fields["prepare_only_fast_fail_used"] = bool(network_evidence.get("prepare_only_fast_fail_used"))
            fields["prepare_only_fast_fail_timeout_ms"] = network_evidence.get("prepare_only_fast_fail_timeout_ms")
            return {
                **fields,
                "confirmed": False,
                "confirmed_by": [],
    ...
    async def _clear_composer_for_trusted_input(self, page: Any, input_locator: Any) -> dict[str, Any]:
        ...
        async def record_phase(name: str, awaitable: Any) -> Any:
            phase_started = time.monotonic()
            try:
                return await awaitable
            finally:
                evidence["phase_timings"][name] = round(time.monotonic() - phase_started, 3)
                evidence["phase_order"].append(name)
        ...
        try:
            ...
            await record_phase("control_a", page.keyboard.press("Control+A"))
            evidence["control_a"] = True
            await record_phase("backspace", page.keyboard.press("Backspace"))
    ...
    async def _paste_prompt_via_clipboard(self, page: Any, input_locator: Any, *, prompt: str) -> dict[str, Any]:
        ...
        try:
            clear_evidence = await record_phase("clear_composer", self._clear_composer_for_trusted_input(page, input_locator))
            evidence["clear_evidence"] = clear_evidence
            context = getattr(page, "context", None)
            if context is not None and hasattr(context, "grant_permissions"):
                try:
                    await record_phase(
                        "clipboard_grant_permissions",
            ...
            await record_phase("clipboard_write_text", page.evaluate("text => navigator.clipboard.writeText(text)", prompt))
            evidence["clipboard_write_used"] = True
            await record_phase("keyboard_control_v", page.keyboard.press("Control+V"))
            evidence["keyboard_paste_used"] = True
            await record_phase("post_paste_dwell", page.wait_for_timeout(100))
        except Exception as exc:
            evidence["error"] = str(exc)
        evidence["duration_seconds"] = round(time.monotonic() - started, 3)
        return evidence
    ...
    async def _insert_prompt_via_keyboard(self, page: Any, input_locator: Any, *, prompt: str) -> dict[str, Any]:
        ...
        try:
            evidence["clear_evidence"] = await record_phase("clear_composer", self._clear_composer_for_trusted_input(page, input_locator))
            await record_phase("keyboard_insert_text", page.keyboard.insert_text(prompt))
            evidence["insert_text_used"] = True
            await record_phase("post_insert_dwell", page.wait_for_timeout(100))
        except Exception as exc:
            evidence["error"] = str(exc)
        evidence["duration_seconds"] = round(time.monotonic() - started, 3)
        return evidence
    ...
    async def _fill_chat_prompt(self, page: Any, input_locator: Any, *, prompt: str) -> dict[str, Any]:
        ...
        evidence: dict[str, Any] = {
            ...
            "timing_decomposition": {
                "diagnostic_fill_path": "v0.0.278.49_retry_fill_timing_decomposition_only",
                "phase_timings": {},
                "phase_order": [],
        ...
        async def record_fill_phase(name: str, awaitable: Any) -> Any:
            phase_started = time.monotonic()
            try:
                return await awaitable
            finally:
                evidence["timing_decomposition"]["phase_timings"][name] = round(time.monotonic() - phase_started, 3)
                evidence["timing_decomposition"]["phase_order"].append(name)
        ...
        async def verify(label: str) -> bool:
            ...
            evidence["react_state_probe"] = verify_evidence
            evidence["timing_decomposition"]["verification_timings"].append(verify_evidence)
            return matched

        if requested_mode == "trusted_paste":
            paste_evidence = await record_fill_phase("trusted_paste_attempt", self._paste_prompt_via_clipboard(page, input_locator, prompt=prompt))
            evidence["attempts"].append({"method": "trusted_paste", **paste_evidence})
            if paste_evidence.get("keyboard_paste_used") and await record_fill_phase("verify_trusted_paste", verify("trusted_paste")):
                duration_seconds = round(time.monotonic() - started, 3)
                evidence["timing_decomposition"]["total_seconds"] = duration_seconds
                evidence.update({
        ...
        if requested_mode in {"trusted_paste", "keyboard_insert_text"}:
            insert_evidence = await record_fill_phase("keyboard_insert_text_attempt", self._insert_prompt_via_keyboard(page, input_locator, prompt=prompt))
            evidence["attempts"].append({"method": "keyboard_insert_text", **insert_evidence})
            if insert_evidence.get("insert_text_used") and await record_fill_phase("verify_keyboard_insert_text", verify("keyboard_insert_text")):
                duration_seconds = round(time.monotonic() - started, 3)
                evidence["timing_decomposition"]["total_seconds"] = duration_seconds
                evidence.update({
        ...
        try:
            await record_fill_phase("locator_fill_attempt", input_locator.fill(prompt, timeout=10_000))
        ...
        evidence["verification_passed"] = await record_fill_phase("verify_locator_fill", verify("locator_fill"))
        evidence["duration_seconds"] = round(time.monotonic() - started, 3)
        evidence["timing_decomposition"]["total_seconds"] = evidence["duration_seconds"]
    ...
    async def _run_keyboard_submit_variant(
        ...
        *,
        ...
        if input_locator is None:
            ...
            return result
        result["diagnostic_submit_path"] = "v0.0.278.49_observational_only_prepare_excluded"
        ...
        try:
            ...
            result["fill_evidence"] = {
                ...
                "timing_decomposition": fill_evidence.get("timing_decomposition"),
                "attempts": fill_evidence.get("attempts"),
                "react_state_probe": fill_evidence.get("react_state_probe"),
            }
            result["fill_timing_decomposition"] = fill_evidence.get("timing_decomposition")
            result["fill_attempts"] = fill_evidence.get("attempts")
