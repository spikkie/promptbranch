...
- Repair release built from `v0.0.245.1`.
- Fixed protocol reply validation for parser-shaped `no_artifact` / `no_change` replies where `parsed["status"]` is `valid` and the actual reply status is exposed as `parsed["reply_status"]` or `parsed["reply"]["status"]`.
- Keeps valid no-artifact protocol smoke replies from failing with false `target_version_mismatch` when no output artifact/version is produced.
...
- Added `lifecycle_classification` with runtime/adopted-source/candidate versions, candidate verdict, checks, warnings, and blockers.
- Kept `mvp-status` read-only: no download, verification write, migration, candidate test, adoption, Project Source mutation, or state advancement.

## v0.0.242

- Added `pb artifact mvp-status` as a read-only Artifact Intake MVP cockpit.
- The command summarizes current artifact state, latest protocol candidate precondition, candidate inventory, MVP completion proof, and the recommended next operator command.
- Kept all lifecycle mutations behind existing explicit commands such as `artifact intake`, `candidate-test`, `accept-candidate`, and `candidate-run`.
- No new Project Source mutation, artifact adoption, download, verification, migration, or test execution is performed by `mvp-status`.
...
- Added `--candidate-mvp-max-steps` and `--candidate-run-step-timeout` controls so the lifecycle loop is bounded and observable.
- Post-release summaries now expose whether candidate-run executed a lifecycle cycle, how many steps ran, why it stopped, and which lifecycle actions were performed.
- Default post-release validation remains plan-only and non-mutating unless `--complete-candidate-mvp` is explicitly supplied.

## v0.0.234

- Wired artifact-candidate MVP completion proof into `scripts/post-release-validation.sh` through `--require-candidate-mvp-complete`.
- The default post-release path still runs `pb artifact candidate-run --json` in plan-only mode; strict completion is opt-in so normal release validation does not require an active accepted candidate record.
- Post-release summaries now surface candidate-run MVP proof fields such as `mvp_complete`, `mvp_completion_status`, and `require_complete`.
...
- `--adopt-if-accepted` mode runs candidate-run after adoption, post-adoption protocol smoke, and artifact-intake dry-run, so the candidate lifecycle command is validated against the adopted baseline context.
- Added `--skip-candidate-run` for rare diagnostics where only lower-level gates should run.
...
- It performs no download, verification write, migration, candidate-test, adoption, Project Source mutation, or artifact/source state advancement.

## v0.0.228

- Added read-only candidate lifecycle inventory reporting with `pb artifact candidate-status --all --json`.
- The inventory reports every registered candidate, lifecycle counts, status counts, and the next recommended MVP command.
- Preserved fail-closed Artifact Intake MVP boundaries: no download, verification write, migration, candidate-test, adoption, Project Source mutation, or baseline advancement is performed by candidate-status.
...
- Adds explicit `performed` fields for adoption and post-adoption semantic checks, so skipped adoption is no longer reported as successful adoption.
- Does not change artifact intake, candidate-test, accept-candidate behavior, Project Source upload, source sync, or MCP policy behavior.

## v0.0.225

- Added `pb artifact candidate-test` as the migrated-candidate test gate.
- The command selects a registered, verified, unaccepted candidate ZIP, re-verifies SHA and ZIP hygiene, and runs release-control in `--tests-only` mode without `--adopt-if-green`.
- Candidate test results are recorded under `.pb_profile/artifact_candidate_tests/` and linked from `.pb_profile/artifact_candidates.json`.
- Adoption, Project Source mutation, artifact-current advancement, and source-current advancement remain disabled in this slice.
...
- Advances Artifact Intake MVP from classification-only to explicit operator-controlled candidate download and ZIP verification.
- `pb artifact intake --from-last-answer --download --verify --json` downloads a validated ZIP candidate to `.pb_profile/artifact_inbox/` and verifies it there only.
- Migration and adoption are intentionally blocked in this MVP slice; `--migrate` now fails closed with `artifact_migration_not_supported_in_mvp`.
- Candidate download now requires ZIP kind, candidate/repair role, `download.available=true`, and an explicit URL.
- Verification reports wrapper-folder, unsafe-entry, nested-ZIP/hygiene, VERSION, filename-version, and expected-version failures without mutating Project Sources or artifact/source state.
...
- Kept full telemetry under `rate_limit_telemetry` while adding operator-facing recommendations for recovered/excessive 429/cooldown runs.
- No protocol schema, source/artifact/adoption behavior, Project Source mutation, or MCP/write policy changed.
...
- Protocol run records are persisted under `.pb_profile/ask_protocol_runs/` for local auditability.
- Artifact candidates are returned for later intake stages, but no artifact download, ZIP verification, migration, adoption, or Project Source mutation is performed.
