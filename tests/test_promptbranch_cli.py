...
def test_main_version_subcommand_outputs_release(capsys) -> None:
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    assert captured.out.strip() == "promptbranch 0.0.278.6"
...
def test_artifact_mvp_status_reports_no_artifact_protocol_precondition(capsys, tmp_path) -> None:
    ...
    assert payload["lifecycle_classification"]["versions"]["runtime_code_version"] == "v0.0.278.6"
...
def test_artifact_mvp_status_warns_when_runtime_differs_from_adopted_source(capsys, tmp_path) -> None:
    ...
    assert classification["versions"]["runtime_code_version"] == "v0.0.278.6"
    ...
    assert plan["versions"]["runtime_code_version"] == "v0.0.278.6"
...
def test_release_lifecycle_status_consolidates_local_state_and_finalizer_summary(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / "VERSION").write_text("v0.0.278.6\n", encoding="utf-8")
    profile = repo / ".pb_profile"
    summary_dir = profile / "release_logs" / "v0.0.278.6"
    summary_dir.mkdir(parents=True)
    summary_path = summary_dir / "post_release_validation.v0.0.278.6.summary.json"
    summary_path.write_text(json.dumps({
        "ok": True,
        "version": "v0.0.278.6",
        "target_version": "v0.0.278.6",
        "failure_count": 0,
    ...
    artifact_name = "chatgpt_claudecode_workflow_v0.0.278.6.zip"
    ...
    store.remember_artifact(
        ...
        artifact_version="v0.0.278.6",
        source_ref=artifact_name,
        source_version="v0.0.278.6",
        project_url="https://chatgpt.com/g/g-p-demo/project",
    )
    backend = _FakeArtifactAdoptBackend(profile, "https://chatgpt.com/g/g-p-demo/project", [])
    args = argparse.Namespace(
    ...
    assert payload["mutating_actions_executed"] is False
    assert payload["runtime"]["runtime_code_version"] == "v0.0.278.6"
    assert payload["version_file"]["normalized_version"] == "v0.0.278.6"
    assert payload["artifact_current"]["baseline_roles"]["adopted_source_version"] == "v0.0.278.6"
...
def test_release_lifecycle_status_text_output_is_human_readable(capsys, tmp_path) -> None:
    ...
    summary_dir = profile / "release_logs" / "v0.0.278.6"
    summary_dir.mkdir(parents=True)
    summary_dir.joinpath("post_release_validation.v0.0.278.6.summary.json").write_text(json.dumps({
        "ok": True,
        "version": "v0.0.278.6",
        "target_version": "v0.0.278.6",
        "failure_count": 0,
    ...
    assert "runtime:           v0.0.278.6" in output
    assert "adopted source:    v0.0.278.6" in output
...
def test_release_doctor_artifact_zip_hardening_reports_candidate_phase(capsys, tmp_path) -> None:
    ...
    subprocess_run = __import__("subprocess").run
    subprocess_run(["git", "init"], cwd=repo, stdout=__import__("subprocess").DEVNULL, stderr=__import__("subprocess").DEVNULL, check=True)
    artifact_path = repo / "chatgpt_claudecode_workflow_v0.0.278.6.zip"
    _write_test_release_zip(artifact_path, "v0.0.278.6")
    profile = tmp_path / "profile"
    project_url = "https://chatgpt.com/g/g-p-demo/project"
    ...
    assert payload["artifact_inspection"]["normalized_version"] == "v0.0.278.6"
...
""".lstrip(), encoding="utf-8")
    (repo / "VERSION").write_text("v0.0.278.6\n", encoding="utf-8")
    artifact = tmp_path / "chatgpt_claudecode_workflow_v0.0.278.6.zip"
    _write_test_release_zip(artifact, "v0.0.278.6")
    ...
    policy = json.loads((repo / ".promptbranch-project.json").read_text(encoding="utf-8"))
    assert policy["accepted_baseline"]["artifact_ref"] == artifact.name
    assert policy["accepted_baseline"]["artifact_version"] == "v0.0.278.6"
...
def test_artifact_mvp_status_reports_completion_after_candidate_acceptance(capsys, tmp_path) -> None:
    filename = "chatgpt_claudecode_workflow_v0.0.278.6.zip"
    repo = tmp_path / "repo"
    repo.mkdir()
    zip_path = repo / filename
    _write_test_release_zip(zip_path, "v0.0.278.6")
    profile = tmp_path / "profile"
    _write_candidate_registry(profile, filename=filename, zip_path=zip_path, version="v0.0.278.6", tested=True)
    backend = _FakeArtifactAdoptBackend(profile, "https://chatgpt.com/g/g-p-demo/project", [])
    ...
    assert payload["lifecycle_classification"]["versions"]["accepted_candidate_version"] == "v0.0.278.6"
    assert payload["mvp_completion"]["accepted_candidate"]["artifact_version"] == "v0.0.278.6"
...
def test_artifact_intake_smoke_zip_verifier_accepts_hello_attachment_zip(tmp_path) -> None:
    ...
    zip_path = inbox / "chatgpt_claudecode_workflow_v0.0.278.6.zip"
    ...
    result = {
        ...
        "selected_candidate": {
            "filename": "chatgpt_claudecode_workflow_v0.0.278.6.zip",
            "filename_version": "v0.0.278.6",
            "expected_version": "v0.0.278.6",
        },
...
def test_artifact_mvp_dod_reports_latest_smoke_zip_evidence(tmp_path, capsys) -> None:
    ...
    inbox.joinpath("intake.json").write_text(json.dumps({
        ...
        "candidate": {"filename": "chatgpt_claudecode_workflow_v0.0.278.6.zip"},
...
def test_artifact_mvp_status_includes_smoke_zip_next_command(tmp_path, capsys) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    repo.joinpath("VERSION").write_text("v0.0.278.6\n", encoding="utf-8")
    profile = tmp_path / ".pb_profile"
    profile.mkdir()
...
def test_candidate_test_parser_default_timeout_is_bounded() -> None:
    parser = make_parser()
    parsed = parser.parse_args(["artifact", "candidate-test", "--version", "v0.0.278.6", "--json"])

    assert parsed.test_timeout == 540.0
    assert parsed.profile == "smoke"

    parsed_full = parser.parse_args(["artifact", "candidate-test", "--version", "v0.0.278.6", "--profile", "full", "--json"])
