...
def test_main_version_subcommand_outputs_release(capsys) -> None:
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    assert captured.out.strip() == "promptbranch 0.0.234"
...
def test_phase1_doctor_reports_state_without_mutating(monkeypatch, capsys, tmp_path) -> None:
    ...
    assert payload["version"] == "0.0.234"
...
def test_artifact_candidate_run_execute_until_blocked_runs_test_then_accept(monkeypatch, capsys, tmp_path) -> None:
    filename = "chatgpt_claudecode_workflow_v0.0.234.zip"
    repo = tmp_path / "repo"
    repo.mkdir()
    zip_path = repo / filename
    _write_test_release_zip(zip_path, "v0.0.234")
    profile = tmp_path / "profile"
    _write_candidate_registry(profile, filename=filename, zip_path=zip_path, version="v0.0.234")
    backend = _FakeArtifactAdoptBackend(profile, "https://chatgpt.com/g/g-p-demo/project", [])
    calls = []
    ...
    def mark_test_passed() -> None:
        ...
        record_dir = profile / "artifact_candidate_tests" / "v0.0.234"
        record_dir.mkdir(parents=True, exist_ok=True)
...
def test_artifact_candidate_run_require_complete_passes_after_acceptance(capsys, tmp_path) -> None:
    ...
    _write_test_release_zip(zip_path, "v0.0.234")
    profile = tmp_path / "profile"
    _write_candidate_registry(profile, filename=filename, zip_path=zip_path, version="v0.0.234", tested=True)
    backend = _FakeArtifactAdoptBackend(profile, "https://chatgpt.com/g/g-p-demo/project", [])
    ...
    assert payload["mvp_completion"]["accepted_candidate"]["artifact_version"] == "v0.0.234"
