...
def test_ask_protocol_print_request_uses_current_baseline(monkeypatch, capsys, tmp_path) -> None:
    ...
    exit_code = main([
        ...
        "ask", "continue v0.0.207",
        "--protocol", "--from-current-baseline", "--target-version", "v0.0.207", "--print-request-json",
    ])
    ...
    assert request["artifact"]["target_version"] == "v0.0.207"
...
def test_ask_protocol_wraps_prompt_before_sending(monkeypatch, capsys, tmp_path) -> None:
    ...
    exit_code = main([
        ...
        "--profile-dir", str(tmp_path),
        "--project-url", project_url,
        "ask", "continue next slice", "--protocol", "--target-version", "v0.0.207",
    ])
    ...
    assert '"target_version": "v0.0.207"' in captured["prompt"]
...
def test_main_version_subcommand_outputs_release(capsys) -> None:
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    assert captured.out.strip() == "promptbranch 0.0.207"
...
def test_phase1_doctor_reports_state_without_mutating(monkeypatch, capsys, tmp_path) -> None:
    ...
    assert payload["version"] == "0.0.207"
...
def test_artifact_intake_rejects_wrong_expected_version(monkeypatch, capsys, tmp_path) -> None:
    ...
    exit_code = main([
        "--service-base-url", "http://localhost:8000",
        "--profile-dir", str(tmp_path),
        "artifact", "intake", "--from-last-answer", "--expect-version", "v0.0.207", "--json",
    ])
...
def test_artifact_accept_candidate_preflight_requires_no_adoption(capsys, tmp_path) -> None:
    filename = "chatgpt_claudecode_workflow_v0.0.207.zip"
    repo = tmp_path / "repo"
    repo.mkdir()
    zip_path = repo / filename
    _write_test_release_zip(zip_path, "v0.0.207")
    script = repo / "chatgpt_claudecode_workflow_release_control.sh"
    script.write_text("#!/usr/bin/env bash\nexit 0\n", encoding="utf-8")
    script.chmod(0o755)
    profile = tmp_path / "profile"
    _write_candidate_registry(profile, filename=filename, zip_path=zip_path, version="v0.0.207")
...
def test_artifact_accept_candidate_runs_release_control_and_marks_candidate_accepted(monkeypatch, capsys, tmp_path) -> None:
    ...
    def fake_run(command, cwd, stdout, stderr, text, timeout, check):
        assert command[:5] == [str(script), "--version", "v0.0.207", "--tests-only", "--adopt-if-green"]
        ...
        backend.store.remember_artifact(
            ...
            artifact_version="v0.0.207",
            source_ref=filename,
            source_version="v0.0.207",
        )
        import subprocess
