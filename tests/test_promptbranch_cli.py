...
def test_main_version_subcommand_outputs_release(capsys) -> None:
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    assert captured.out.strip() == "promptbranch 0.0.247"
...
def test_phase1_doctor_reports_state_without_mutating(monkeypatch, capsys, tmp_path) -> None:
    ...
    assert payload["version"] == "0.0.247"
...
def test_artifact_mvp_status_reports_no_artifact_protocol_precondition(capsys, tmp_path) -> None:
    ...
    assert payload["lifecycle_classification"]["versions"]["runtime_code_version"] == "v0.0.247"
...
def test_artifact_mvp_status_warns_when_runtime_differs_from_adopted_source(capsys, tmp_path) -> None:
    ...
    assert classification["versions"]["runtime_code_version"] == "v0.0.247"
    ...
    assert plan["versions"]["runtime_code_version"] == "v0.0.247"
...
def test_release_doctor_reports_runtime_source_mismatch_read_only(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / "VERSION").write_text("v0.0.247\n", encoding="utf-8")
    ...
    assert payload["read_only"] is True
    assert payload["mutating_actions_executed"] is False
    assert payload["runtime"]["runtime_code_version"] == "v0.0.247"
    assert payload["version_file"]["normalized_version"] == "v0.0.247"
...
def test_release_doctor_artifact_zip_hardening_reports_candidate_phase(capsys, tmp_path) -> None:
    ...
    subprocess_run = __import__("subprocess").run
    subprocess_run(["git", "init"], cwd=repo, stdout=__import__("subprocess").DEVNULL, stderr=__import__("subprocess").DEVNULL, check=True)
    artifact_path = repo / "chatgpt_claudecode_workflow_v0.0.247.zip"
    _write_test_release_zip(artifact_path, "v0.0.247")
    profile = tmp_path / "profile"
    project_url = "https://chatgpt.com/g/g-p-demo/project"
    ...
    assert payload["artifact_inspection"]["normalized_version"] == "v0.0.247"
...
def test_artifact_mvp_status_reports_completion_after_candidate_acceptance(capsys, tmp_path) -> None:
    filename = "chatgpt_claudecode_workflow_v0.0.247.zip"
    repo = tmp_path / "repo"
    repo.mkdir()
    zip_path = repo / filename
    _write_test_release_zip(zip_path, "v0.0.247")
    profile = tmp_path / "profile"
    _write_candidate_registry(profile, filename=filename, zip_path=zip_path, version="v0.0.247", tested=True)
    backend = _FakeArtifactAdoptBackend(profile, "https://chatgpt.com/g/g-p-demo/project", [])
    ...
    assert payload["lifecycle_classification"]["versions"]["accepted_candidate_version"] == "v0.0.247"
    assert payload["mvp_completion"]["accepted_candidate"]["artifact_version"] == "v0.0.247"
