...
def _seed_protocol_project_scope(
    monkeypatch,
    tmp_path: Path,
    *,
    project_url: str,
    ...
    repo_id = artifact_ref.rsplit("_v", 1)[0]
...
def test_main_can_ask_via_service_backend(monkeypatch, capsys, tmp_path) -> None:
    ...
    assert payload["answer_text"] == "world"


def test_text_ask_opts_into_answer_only_output(monkeypatch, capsys, tmp_path) -> None:
    class FakeServiceClient:
        def __init__(self, base_url: str, *, token: str | None = None, timeout: float = 900.0) -> None:
            pass
...
def test_default_ask_emits_failure_payload_and_exits_nonzero(monkeypatch, capsys, tmp_path) -> None:
    class FakeServiceClient:
        def __init__(self, base_url: str, *, token: str | None = None, timeout: float = 900.0) -> None:
            pass
    ...
    assert payload["status"] == "submit_causality_not_confirmed"
    assert payload["answer_text"] == ""


def test_plain_ask_renders_all_answer_paragraphs(monkeypatch, capsys, tmp_path) -> None:
    paragraphs = [
        f"{idx}. FULL_ANSWER_SENTINEL paragraph {idx}"
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    assert captured.out.count("FULL_ANSWER_SENTINEL") == 5
    assert captured.out.strip() == "\n\n".join(paragraphs)


def test_json_ask_exposes_canonical_answer_text_for_paragraphs(monkeypatch, capsys, tmp_path) -> None:
    paragraphs = [
        f"{idx}. FULL_ANSWER_SENTINEL paragraph {idx}"
        for idx in range(1, 6)
    ]
    ...
    assert payload["answer_text"].count("FULL_ANSWER_SENTINEL") == 5
    assert payload["answer_text_length"] == len(answer_text)
    assert payload["answer_text_sha256"] == hashlib.sha256(answer_text.encode("utf-8")).hexdigest()
    assert payload["answer_paragraph_count"] == 5
    assert payload["answer"]["text"] == answer_text
...
def test_ask_protocol_print_request_uses_current_baseline(monkeypatch, capsys, tmp_path) -> None:
    ...
    write_repo_identity(
        tmp_path,
        project_id="g-p-demo-claude-code",
        project_home_url=project_url,
        repo_id="chatgpt_claudecode_workflow",
        artifact_pattern="chatgpt_claudecode_workflow_<version>.zip",
        role="release_authority",
    )
    identity = load_repo_identity(tmp_path)
    assert identity is not None
    ...
    registry.add(ArtifactRecord(
        path=str(tmp_path / "chatgpt_claudecode_workflow_v0.0.206.zip"),
        filename="chatgpt_claudecode_workflow_v0.0.206.zip",
        kind="adopted_release",
        version="v0.0.206",
        repo_path=None,
        repo_id="chatgpt_claudecode_workflow",
...
def test_ask_protocol_wraps_prompt_before_sending(monkeypatch, capsys, tmp_path) -> None:
    ...
    exit_code = main([
        ...
        "--profile-dir", str(tmp_path),
        "--project-url", project_url,
        "ask", "--text", "continue next slice", "--protocol", "--target-version", "v0.0.210",
    ])
...
def test_ask_protocol_parse_reply_validates_request_and_baseline(monkeypatch, capsys, tmp_path) -> None:
    ...
    assert payload["artifact_candidates"][0]["source"]["request_id"] == "req-v210"
    assert payload["artifact_candidates"][0]["source"]["answer_id"] == "a-new"
...
def test_protocol_service_timeout_floor_covers_browser_response_wait(monkeypatch) -> None:
    from promptbranch_cli import _protocol_service_timeout_floor

    monkeypatch.delenv("CHATGPT_RESPONSE_TIMEOUT_MS", raising=False)
    args = argparse.Namespace(protocol_timeout_seconds=120.0, protocol_fresh_turn_timeout_seconds=60.0)

    assert _protocol_service_timeout_floor(args) >= 1350.0


def test_protocol_service_timeout_floor_uses_response_timeout_env(monkeypatch) -> None:
    from promptbranch_cli import _protocol_service_timeout_floor

    monkeypatch.setenv("CHATGPT_RESPONSE_TIMEOUT_MS", "1500000")
    args = argparse.Namespace(protocol_timeout_seconds=120.0, protocol_fresh_turn_timeout_seconds=12.0)

    assert _protocol_service_timeout_floor(args) >= 1602.0
...
def test_main_version_subcommand_outputs_release(capsys) -> None:
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    assert captured.out.strip() == f"promptbranch {_TEST_PACKAGE_VERSION}"
...
def test_ask_live_profile_runs_visible_operator_steps_in_unique_delete_frozen_project(monkeypatch, capsys) -> None:
    ...
    test_project_url = f"https://chatgpt.com/g/{project_id}/project"
    test_conversation_url = f"https://chatgpt.com/g/{project_id}-ask-live-temp-unit/c/00000000-0000-0000-0000-000000000001"

    class FakeBackend:
        def state_snapshot(self):
            return {
                "current_project_home_url": "https://chatgpt.com/g/g-p-original/project",
                "current_conversation_url": "https://chatgpt.com/g/g-p-original/c/original",
        ...
        async def ask(self, *, prompt: str, attachment_paths=None, conversation_url=None, expect_json=False, keep_open=False, retries=None, file_path=None, prefer_button_submit=False):
            ...
            return {"ok": True, "answer": sentinel, "conversation_url": test_conversation_url}

    monkeypatch.setattr("promptbranch_cli.build_backend", lambda args: FakeBackend())

    rc = main(["test", "ask-live", "--json", "--run-id", "UNIT", "--only", "plain,prompt_file"])

    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["ok"] is True
    assert payload["profile"] == "ask-live"
    assert payload["mode"] == "visible_local_debug_browser"
...
def test_visual_artifact_roundtrip_wraps_ask_and_artifact_intake(monkeypatch, capsys, tmp_path) -> None:
    ...
    class FakeBackend:
        ...
        async def ask(self, *, prompt: str, attachment_paths=None, conversation_url=None, expect_json=False, keep_open=False, retries=None, file_path=None, prefer_button_submit=False):
            ...
            assert '"status": "completed"' not in prompt
            assert "BEGIN_PROMPTBRANCH_REPLY_JSON\n{" not in prompt
            assert attachment_paths and str(attachment_paths[0]).endswith("pb_visual_artifact_roundtrip_input_UNIT.zip")
            envelope = {
                "schema": "promptbranch.ask.reply",
                "schema_version": "1.0",
                "request_id": "visual-artifact-roundtrip-UNIT",
                "correlation_id": "visual-artifact-roundtrip-UNIT",
                "status": "completed",
                ...
                "baseline": {
                    "input_artifact": None,
                    "input_version": None,
                    "output_artifact": "pb_visual_artifact_roundtrip_UNIT.zip",
                    "output_version": None,
                ...
                "artifacts": [{
                    "kind": "zip",
                    "filename": "pb_visual_artifact_roundtrip_UNIT.zip",
                    "version": None,
                    "role": "visual_artifact_roundtrip_output",
                    "download": {
                        "available": True,
                        "link_text": "pb_visual_artifact_roundtrip_UNIT.zip",
    ...
    assert rc == 0
    assert payload["ok"] is True
    assert payload["profile"] == "visual-artifact-roundtrip"
    assert payload["status"] == "verified"
    assert payload["debug_browser"] is True
    assert payload["service_transport_used"] is False
...
def test_visual_artifact_roundtrip_failure_payload_waits_for_temp_project_cleanup(monkeypatch, capsys, tmp_path) -> None:
    calls: list[dict[str, object]] = []

    class FakeBackend:
        def __init__(self) -> None:
            self.project_url = "https://chatgpt.com/g/g-p-33333333333333333333333333333333-visual-artifact-roundtrip-temp-fail/project"
        ...
        async def ask(self, *, prompt: str, attachment_paths=None, conversation_url=None, expect_json=False, keep_open=False, retries=None, file_path=None, prefer_button_submit=False):
            ...
            return {
                "ok": False,
                "status": "simulated_ask_failure",
                "error": "simulated ask failure",
                "error_type": "SimulatedAskFailure",
            }
...
def test_ask_live_project_id_accepts_slugged_chatgpt_project_url() -> None:
    from promptbranch_cli import _ask_live_in_expected_project, _ask_live_project_id_from_url

    project_id = "g-p-6a1af3fe64a481919a2cc7de3cff0487"
    bare_home = f"https://chatgpt.com/g/{project_id}/project"
    slugged_home = f"https://chatgpt.com/g/{project_id}-ask-live-temp-20260530t142729z/project"
    slugged_conversation = f"https://chatgpt.com/g/{project_id}-ask-live-temp-20260530t142729z/c/00000000-0000-0000-0000-000000000001"

    assert _ask_live_project_id_from_url(bare_home) == project_id
    assert _ask_live_project_id_from_url(slugged_home) == project_id
    assert _ask_live_project_id_from_url(slugged_conversation) == project_id

    in_expected, expected_id, response_id = _ask_live_in_expected_project(
        expected_project_home_url=bare_home,
        response_project_home_url=slugged_home,
    )

    assert in_expected is True
    assert expected_id == project_id
    assert response_id == project_id
...
def test_phase1_doctor_reports_state_without_mutating(monkeypatch, capsys, tmp_path) -> None:
    ...
    assert payload["version"] == _TEST_PACKAGE_VERSION
...
def test_artifact_intake_protocol_run_downloads_verifies_and_migrates_without_live_chat(monkeypatch, capsys, tmp_path) -> None:
    ...
    class BrowserDownloadServiceClient:
        def __init__(self, *args, **kwargs):
            pass
        ...
        def download_chat_artifact(self, conversation_url, *, artifact_url, filename, target_path, timeout_seconds=120.0, keep_open=False, project_url=None):
    ...
    monkeypatch.setattr("promptbranch_cli.ChatGPTServiceClient", BrowserDownloadServiceClient)
    exit_code = main([
        "--service-base-url", "http://localhost:8000",
        "--profile-dir", str(tmp_path),
...
def test_artifact_intake_sandbox_downloads_through_browser_session(monkeypatch, capsys, tmp_path) -> None:
    ...
    assert payload["download_performed"] is True
    assert payload["browser_download_performed"] is True
    assert payload["download_transport"]["status"] == "artifact_browser_downloaded"
    assert payload["download_transport"]["scheme"] == "sandbox"
    assert payload["download_transport"]["browser_result"]["status"] == "artifact_browser_downloaded"
    assert Path(payload["download"]["path"]).is_file()
...
def test_artifact_candidate_next_ignores_stale_accepted_missing_zip_candidate(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    repo, profile = _initialize_test_project_scope(tmp_path, repo_id="chatgpt_claudecode_workflow", repo_root=repo)
    project_url = "https://chatgpt.com/g/g-p-demo/project"

    current_filename = "chatgpt_claudecode_workflow_v0.0.275.zip"
    current_zip = repo / current_filename
    _write_test_release_zip(current_zip, "v0.0.275")
    backend = _FakeArtifactAdoptBackend(profile, project_url, [{"title": current_filename, "id": "src_current"}])
    adopt_args = argparse.Namespace(
        artifact=current_filename,
        from_project_source=True,
        local_path=str(current_zip),
        repo_path=str(repo),
        keep_open=False,
        json=True,
        profile_dir=str(profile),
    )
    assert asyncio.run(cmd_artifact_adopt(backend, adopt_args)) == 0
    capsys.readouterr()

    stale_filename = "chatgpt_claudecode_workflow_v0.0.259.zip"
    stale_zip = repo / stale_filename
    _write_test_release_zip(stale_zip, "v0.0.259")
    stale_candidate = _write_candidate_registry(profile, filename=stale_filename, zip_path=stale_zip, version="v0.0.259", tested=True)
    stale_candidate["accepted"] = True
    stale_candidate["adoption_performed"] = True
    stale_candidate["status"] = "candidate_release"
    stale_zip.unlink()
    (profile / "artifact_candidates.json").write_text(
        json.dumps({"schema_version": 1, "candidates": [stale_candidate]}, indent=2) + "\n",
        encoding="utf-8",
    )
    ...
    assert exit_code == 0
    assert payload["ok"] is True
    assert payload["status"] == "candidate_next_no_actionable_candidate"
    assert payload["candidate_count"] == 1
    assert payload["actionable_candidate_count"] == 0
    assert payload["historical_or_non_actionable_candidate_count"] == 1
    assert payload["selected_candidate"] is None
    assert payload["recommended_next_command"]["kind"] == "no_actionable_candidate"
    assert payload["recommended_next_command"]["command"] is None
    assert "repair_or_remigrate_candidate" not in payload["recommended_next_command"].get("kind", "")
    assert payload["inventory"]["status"] == "candidate_inventory_history_only"
    assert payload["inventory"]["status_counts"] == {"candidate_already_accepted": 1}
    historical = payload["inventory"]["candidates"][0]
    assert historical["recommended_next_command"]["kind"] == "candidate_already_accepted"
    assert historical["recommended_next_command"]["command"] == "pb artifact current --json"
    assert historical["recommended_next_command"]["kind"] != "repair_or_remigrate_candidate"
    assert payload["artifact_current"]["repos"]["chatgpt_claudecode_workflow"]["state"]["artifact_version"] == "v0.0.275"
    assert payload["mutating_actions_executed"] is False
    assert payload["project_source_mutated"] is False
...
def test_artifact_candidate_run_plans_one_allowlisted_next_step_without_mutation(capsys, tmp_path) -> None:
    ...
    assert payload["accept_if_green"] is False
    assert payload["mutating_actions_executed"] is False
    assert payload["candidate_test_performed"] is False
    assert payload["adoption_performed"] is False
    assert payload["project_source_mutated"] is False
...
def test_release_lifecycle_status_consolidates_local_state_and_finalizer_summary(capsys, tmp_path) -> None:
    ...
    summary_dir = profile / "release_logs" / "v0.1.0"
    summary_dir.mkdir(parents=True)
    summary_path = summary_dir / "post_release_validation.v0.1.0.summary.json"
...
def test_release_evidence_status_reports_green_post_release_summary(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    ...
    summary_dir.mkdir(parents=True)
    summary_path = summary_dir / f"post_release_validation.{runtime_version}.summary.json"
    summary_path.write_text(json.dumps({
        "ok": True,
        "version": runtime_version,
        "target_version": runtime_version,
    ...
    args = argparse.Namespace(version=runtime_version, repo_path=str(repo), json=True, profile_dir=str(profile))
    exit_code = asyncio.run(cmd_release_evidence_status(FakeBackend(), args))
    payload = json.loads(capsys.readouterr().out)

    assert exit_code == 0
    assert payload["action"] == "release_evidence_status"
    assert payload["read_only"] is True
    assert payload["status"] == "full_test_evidence_green"
    assert payload["full_test_evidence"]["full_test_green"] is True
    assert payload["full_test_evidence"]["source_kind"] == "post_release_validation_summary"
    assert payload["full_test_evidence"]["evidence_path"] == str(summary_path.resolve())
    assert payload["mutating_actions_executed"] is False
    assert payload["full_test_performed"] is False


def test_release_baseline_status_embeds_full_test_evidence(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    runtime_version = _test_runtime_version()
    (repo / "VERSION").write_text(runtime_version + "\n", encoding="utf-8")
...
""".lstrip(), encoding="utf-8")
    ...
    assert exit_code == 0
    assert payload["full_test_evidence"]["full_test_green"] is True
    assert payload["full_test_evidence"]["status"] == "full_test_green"
    assert payload["full_test_evidence"]["source_kind"] == "post_release_validation_summary"
...
def test_release_lifecycle_status_text_output_is_human_readable(capsys, tmp_path) -> None:
    ...
    summary_dir = profile / "release_logs" / "v0.1.0"
    summary_dir.mkdir(parents=True)
    summary_dir.joinpath("post_release_validation.v0.1.0.summary.json").write_text(json.dumps({
        "ok": True,
        "version": "v0.1.0",
        "target_version": "v0.1.0",
        "failure_count": 0,
    ...
    assert exit_code == 0
    assert "Release lifecycle status" in output
    ...
    assert "runtime:           v0.1.0" in output
    assert "adopted source:    v0.1.0" in output
...
def test_release_status_guide_selects_checkpoint_and_full_test_runbook_at_threshold(capsys, tmp_path) -> None:
    ...
    assert exit_code == 0
    assert payload["schema"] == "promptbranch.release.status_guide"
    assert payload["detected_context"] == "development_candidate"
    assert payload["primary_read_command_kind"] == "checkpoint"
    assert payload["baseline_status_applicable"] is False
    assert payload["primary_read_command"] == payload["command_guide"]["development_checkpoint"]
...
def test_release_status_guide_selects_baseline_status_for_adopted_current(capsys, tmp_path) -> None:
    ...
    assert exit_code == 0
    assert payload["detected_context"] == "post_adoption_baseline"
...
""".lstrip(), encoding="utf-8")
    ...
    dev_head = repo / "chatgpt_claudecode_workflow-2_v0.1.5.zip"
    _write_test_release_zip(accepted, "v0.1.2")
    _write_test_release_zip(dev_head, "v0.1.5")
    ...
    exit_code = asyncio.run(cmd_release_dev_status(FakeBackend(), args))
    payload = json.loads(capsys.readouterr().out)

    assert exit_code == 0
    assert payload["ok"] is True
    assert payload["action"] == "release_dev_status"
    assert payload["read_only"] is True
    assert payload["status"] == "development_head_ahead_of_accepted"
    assert payload["accepted_baseline"]["source_version"] == "v0.1.2"
    assert payload["runtime"]["version"] == _test_runtime_version()
    assert payload["development_head_version"] == "v0.1.5"
    assert payload["version_plan"]["expected_next_normal_from_accepted"] == "v0.1.3"
    assert payload["version_plan"]["next_development_version"] == "v0.1.6"
    assert "runtime_ahead_of_accepted_baseline" in payload["warning_codes"]
    assert "dev_head_skips_next_normal_from_accepted" in payload["warning_codes"]
    assert payload["mutating_actions_executed"] is False
    assert payload["adoption_performed"] is False


def test_release_dev_status_explicit_artifact_becomes_dev_head(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    config = repo / ".promptbranch-release.yml"
    config.write_text("""
...
""".lstrip(), encoding="utf-8")
    accepted = repo / "chatgpt_claudecode_workflow-2_v0.1.2.zip"
    explicit = tmp_path / "chatgpt_claudecode_workflow-2_v0.1.5.zip"
    _write_test_release_zip(accepted, "v0.1.2")
    ...
    assert exit_code == 0
    assert payload["development_head"]["selection"] == "explicit_artifact"
    assert payload["development_head"]["artifact_verified"] is True
    assert payload["development_head_version"] == "v0.1.5"
    assert payload["explicit_artifact"]["filename"] == explicit.name
    assert payload["project_source_mutated"] is False
...
""".lstrip(), encoding="utf-8")
    ...
    assert exit_code == 0
    assert payload["schema"] == "promptbranch.release.checkpoint"
    assert payload["action"] == "release_checkpoint"
    assert payload["status"] == "continue_development_ready"
    assert payload["read_only"] is True
    ...
    assert payload["contextualized_warnings"][0]["contextualization_reason"] == "explicit_development_candidate_continue_mode"
    assert payload["install_plan_summary"]["contextualized_warning_codes"] == ["release_install_candidate_not_next_normal_version"]
...
def test_release_checkpoint_adopt_mode_preserves_not_next_normal_warning(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    config = repo / ".promptbranch-release.yml"
    config.write_text("""
...
""".lstrip(), encoding="utf-8")
    ...
    assert exit_code == 0
    assert payload["status"] == "full_test_required_before_adoption"
    assert "release_install_candidate_not_next_normal_version" in payload["warning_codes"]
    assert payload["contextualized_warning_codes"] == []
...
""".lstrip(), encoding="utf-8")
    ...
    assert exit_code == 0
    assert payload["status"] == "full_test_required_before_adoption"
    assert payload["checkpoint_decision"]["recommendation"] == "run_full_test_then_adopt_if_green"
    assert payload["checkpoint_decision"]["run_full_test_now"] is True
    assert payload["checkpoint_decision"]["adopt_now"] is False
    assert "release_checkpoint_full_test_required" in payload["warning_codes"]
    assert payload["suggested_commands"]["full_test_before_adoption"]
    assert payload["mutating_actions_executed"] is False
...
""".lstrip(), encoding="utf-8")
    (repo / "VERSION").write_text("v0.1.0\n", encoding="utf-8")
    artifact = tmp_path / "chatgpt_claudecode_workflow_v0.1.0.zip"
    _write_test_release_zip(artifact, "v0.1.0")
    ...
    policy = json.loads((repo / ".promptbranch-project.json").read_text(encoding="utf-8"))
    assert policy["accepted_baseline"]["artifact_ref"] == artifact.name
    assert policy["accepted_baseline"]["artifact_version"] == "v0.1.0"
...
def test_artifact_mvp_dod_reports_canonical_document_visible(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    docs = repo / "docs"
    docs.mkdir(parents=True)
    source_doc = Path(__file__).resolve().parents[1] / "docs" / "mvp-definition-of-done.md"
    (docs / "mvp-definition-of-done.md").write_text(source_doc.read_text(encoding="utf-8"), encoding="utf-8")
    args = argparse.Namespace(repo_path=str(repo), json=True)

    exit_code = asyncio.run(cmd_artifact_mvp_dod(None, args))
    payload = json.loads(capsys.readouterr().out)

    assert exit_code == 0
    assert payload["ok"] is True
    assert payload["status"] == "mvp_dod_visible"
    report = payload["mvp_definition_of_done"]
    assert report["path"] == "docs/mvp-definition-of-done.md"
    assert report["exists"] is True
    assert report["missing_sections"] == []
    assert report["missing_workflow_markers"] == []
    assert report["final_verdict_rule_present"] is True
    assert payload["read_only"] is True
    assert payload["mutating_actions_executed"] is False


def test_artifact_mvp_dod_reports_missing_document(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    args = argparse.Namespace(repo_path=str(repo), json=True)
    ...
    assert exit_code == 1
    assert payload["ok"] is False
    assert payload["status"] == "mvp_dod_missing"
    assert payload["mvp_definition_of_done"]["exists"] is False
    assert payload["read_only"] is True
    assert payload["mutating_actions_executed"] is False
...
def test_artifact_mvp_status_reports_completion_after_candidate_acceptance(capsys, tmp_path) -> None:
    ...
    assert payload["mvp_definition_of_done"]["status"] in {"mvp_dod_visible", "mvp_dod_missing"}
    assert payload["mutating_actions_executed"] is False
...
def test_artifact_candidate_run_require_real_candidate_accepts_scoped_adopted_current(capsys, tmp_path) -> None:
    ...
    project_url = "https://chatgpt.com/g/g-p-demo/project"
    backend = _FakeArtifactAdoptBackend(profile, project_url, [])
    artifact_name = "chatgpt_claudecode_workflow_v0.0.274.zip"
    backend.store.remember_artifact(
        artifact_ref=artifact_name,
        artifact_version="v0.0.274",
        source_ref=artifact_name,
        source_version="v0.0.274",
        project_url=project_url,
    )
    registry_path = profile / "promptbranch_artifacts.json"
    registry_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            "artifacts": [
                {
                    ...
                    "version": "v0.0.257",
                    "role": "candidate_release",
                    "download": {"available": True, "url": "https://example.test/chatgpt_claudecode_workflow_v0.0.257.zip"},
                }
            ],
        },
    }
...
def test_ask_release_validation_rejects_json_only_sandbox_artifact() -> None:
    from promptbranch_cli import _validate_ask_release_candidate_result
            ...
            "artifacts": [
                {
                    ...
                    "download": {
                        "available": True,
                        "link_text": "chatgpt_claudecode_workflow_v0.0.257.zip",
                        "url": "sandbox:/mnt/data/chatgpt_claudecode_workflow_v0.0.257.zip",
                    },
                }
            ],
        },
    ...
    assert payload["ok"] is False
    assert payload["status"] == "artifact_declared_but_not_attached"
    assert payload["artifact_materialization_proven"] is False
    assert payload["manual_import_required"] is True
    assert "ask_release:artifact_declared_but_not_attached" in payload["reply_validation_errors"]
    assert payload["ask_release_validation"]["checks"]["download_json_only_declared"] is True
...
def test_task_answer_parse_resolves_unique_answer_id_globally(monkeypatch, capsys, tmp_path) -> None:
    ...
    reply = {
        ...
        "baseline": {
            "input_artifact": "chatgpt_claudecode_workflow_v0.0.274.zip",
            "input_version": "v0.0.274",
            "output_artifact": "chatgpt_claudecode_workflow_v0.0.274.zip",
            "output_version": "v0.0.274",
            "target_version": "v0.0.274",
            "release_type": "normal",
        },
        "changes": [],
        "artifacts": [
            {
                "kind": "zip",
                "filename": "chatgpt_claudecode_workflow_v0.0.274.zip",
                "version": "v0.0.274",
                "role": "candidate_release",
                "download": {"available": True, "url": "sandbox:/mnt/data/chatgpt_claudecode_workflow_v0.0.274.zip"},
            }
        ],
...
def test_artifact_intake_smoke_zip_verifier_accepts_hello_attachment_zip(tmp_path) -> None:
    profile_dir = tmp_path / ".pb_profile"
    inbox = profile_dir / "artifact_inbox" / "conversation-1" / "answer-1" / "req-smoke"
    inbox.mkdir(parents=True)
    zip_path = inbox / "chatgpt_claudecode_workflow_v0.1.0.zip"
    ...
    verified = _verify_intake_smoke_zip_candidate(
        ...
        answer_id="answer-1",
        expected_entries=["hello.txt"],
        expected_content="durable ChatGPT UI attachment smoke test",
    )

    assert verified["ok"] is True
    assert verified["status"] == "smoke_zip_verified"
    assert verified["verification_performed"] is True
    assert verified["smoke_verification_performed"] is True
    assert verified["release_verification_performed"] is False
    assert verified["smoke_verification"]["entries"] == ["hello.txt"]
    assert verified["smoke_verification"]["content_checks"]["hello.txt"]["ok"] is True
    assert "VERSION" not in verified["smoke_verification"]["entries"]


def test_artifact_intake_strict_release_verifier_still_rejects_smoke_zip_without_version(tmp_path) -> None:
    profile_dir = tmp_path / ".pb_profile"
    inbox = profile_dir / "artifact_inbox" / "conversation-1" / "answer-1" / "req-smoke"
    inbox.mkdir(parents=True)
    zip_path = inbox / "chatgpt_claudecode_workflow_v0.1.0.zip"
    ...
    assert verified["status"] == "artifact_version_file_missing"
    assert "artifact_version_file_missing" in verified["verification_errors"]


def test_artifact_mvp_dod_reports_latest_smoke_zip_evidence(tmp_path, capsys) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    docs = repo / "docs"
    docs.mkdir()
    docs.joinpath("mvp-definition-of-done.md").write_text(
        "\n".join([
            ...
            "pb artifact intake --download --verify --verify-smoke-zip --expect-entry --expect-content --migrate",
            "scripts/finalize-artifact-intake-mvp.sh --require-real-candidate-mvp",
            "pb artifact current",
            "pb ask --from-current-baseline",
            "## Required invariants",
            "## Required failure behavior",
            "## Required evidence artifacts",
            "## Out of scope for MVP",
            "## Final MVP verdict rule",
            "twice in a row with no manual ZIP repair",
        ])
        + "\n",
        encoding="utf-8",
    )
    ...
    inbox.joinpath("intake.json").write_text(json.dumps({
        ...
        "candidate": {"filename": "chatgpt_claudecode_workflow_v0.1.0.zip"},
        "download": {"path": str(zip_path), "size_bytes": zip_path.stat().st_size},
        "smoke_verification": {
            "ok": True,
            "status": "smoke_zip_verified",
            "entries": ["hello.txt"],
            "content_checks": {"hello.txt": {"ok": True, "status": "matched"}},
        },
        "reply_request_id": "req-smoke",
        "conversation_id": "conversation-1",
        "answer_id": "answer-1",
    ...
    assert exit_code == 0
    assert payload["ok"] is True
    assert payload["mvp_definition_of_done"]["ok"] is True
    assert payload["smoke_zip_verification"]["ok"] is True
    assert payload["smoke_zip_verification"]["latest"]["status"] == "smoke_zip_verified"
...
def test_artifact_mvp_status_includes_smoke_zip_next_command(tmp_path, capsys, monkeypatch) -> None:
    ...
    assert exit_code == 0
    assert "smoke_zip_verification" in payload
    assert payload["smoke_zip_verification"]["status"] == "smoke_zip_evidence_missing"
    assert "--verify-smoke-zip" in payload["commands"]["verify_smoke_zip"]


def test_candidate_test_runner_times_out_with_structured_logs(tmp_path) -> None:
    ...
    assert result["status"] == "candidate_test_timeout"
    assert result["timeout_seconds"] == 1.0
    assert result["process_group_pid"] == result["pid"]
    assert result["stdout_log_path"] == str(stdout_log.resolve())
    assert "start" in result["stdout_tail"]
    assert result["adoption_performed"] is False


def test_candidate_test_parser_default_timeout_is_bounded() -> None:
    parser = make_parser()
    parsed = parser.parse_args(["artifact", "candidate-test", "--version", "v0.1.0", "--json"])

    assert parsed.test_timeout == 540.0
    assert parsed.profile == "smoke"

    parsed_full = parser.parse_args(["artifact", "candidate-test", "--version", "v0.1.0", "--profile", "full", "--json"])
...
def test_promptbranch_smoke_substep_timeout_reports_json_shape(tmp_path) -> None:
    ...
    assert result["ok"] is False
    assert result["status"] == "smoke_substep_timeout"
    assert result["timeout_seconds"] == 1.0
    assert result["process_group_pid"] == result["pid"]
    assert "smoke-start" in result["stdout_tail"]
...
def test_browser_status_command_uses_service_client(monkeypatch, capsys) -> None:
    class FakeServiceClient:
        def __init__(self, base_url: str, *, token: str | None = None, timeout: float = 900.0) -> None:
            pass
    ...
    exit_code = main(["--service-base-url", "http://localhost:8000", "browser", "status", "--json"])
...
def test_ask_live_failed_response_does_not_report_sentinel_match(monkeypatch) -> None:
    ...
    class FakeBackend:
        async def ask(self, **kwargs):
            return {
                ...
                "action": "ask",
                "status": "submit_causality_not_confirmed",
                "error": "ASK_LIVE_FILE_ATTACHMENT_UNIT appeared in an internal error payload only",
                "conversation_url": None,
            }
    ...
    result = asyncio.run(_run_ask_live_step(
        ...
        expected_sentinel="ASK_LIVE_FILE_ATTACHMENT_UNIT",
        attachment_paths=["/tmp/ask_live_attachment.txt"],
        expected_project_home_url="https://chatgpt.com/g/g-p-6a1af3fe64a481919a2cc7de3cff0487/project",
    ))

    assert result["ok"] is False
    assert result["status"] == "ask_failed"
...
def test_parallel_ask_plan_builds_protocol_requests_and_serializes_same_conversation(capsys, tmp_path) -> None:
    from promptbranch_cli import cmd_parallel
    ...
    async def run() -> int:
        ...
        args = argparse.Namespace(
            ...
            request_prefix="req_parallel_test",
            keep_open=False,
            json=True,
            profile_dir=str(tmp_path),
        )
    ...
    assert rc == 0
    assert payload["action"] == "parallel_ask_plan"
    assert payload["status"] == "planned"
    assert payload["automation_performed"] is False
    ...
    assert payload["serialized_group_count"] == 1
    assert payload["concurrency"] == 2
...
def test_release_reconcile_current_reports_stale_current_and_recommended_adopt_command(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    artifact = repo / "chatgpt_claudecode_workflow-2_v0.1.50.zip"
    _write_test_release_zip(artifact, "v0.1.50")
    ...
    exit_code = asyncio.run(cmd_release_reconcile_current(FakeBackend(), args))
    payload = json.loads(capsys.readouterr().out)

    assert exit_code == 1
    assert payload["status"] == "reconciliation_required"
    assert payload["has_stale_baseline"] is True
    assert payload["release_lifecycle_execution_blocked_until_reconciled"] is True
    assert payload["current_versions"]["registry_current_version"] == "v0.1.40"
    assert payload["artifact_version"] == "v0.1.50"
    assert payload["recommended_commands"]["adopt_current_artifact"] == (
        f"pb artifact adopt {artifact.name} --from-project-source --local-path {artifact} --json"
    )
    assert payload["mutating_actions_executed"] is False


def test_release_lifecycle_plan_blocks_when_artifact_current_is_stale(capsys, tmp_path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".promptbranch-release.yml").write_text(
        """
...
""".lstrip(),
    ...
    args = argparse.Namespace(
        ...
        version="v0.1.50",
        target_version="v0.1.51",
        config=str(repo / ".promptbranch-release.yml"),
        repo_path=str(repo),
        plan=True,
        commit=False,
        push=False,
    ...
    exit_code = asyncio.run(cmd_release_lifecycle(FakeBackend(), args))
    payload = json.loads(capsys.readouterr().out)

    assert exit_code == 1
    assert payload["status"] == "blocked"
    assert "release_current_reconciliation_required" in payload["blocker_codes"]
...
def test_current_adopted_candidate_fallback_reads_repo_loop_payload() -> None:
    from promptbranch_cli import _current_adopted_candidate_fallback
    ...
    fallback = _current_adopted_candidate_fallback(
        payload,
        requested_artifact="chatgpt_claudecode_workflow-2_v0.1.76.zip",
        requested_version="v0.1.76",
    )

    assert fallback is not None
    assert fallback["artifact_ref"] == "chatgpt_claudecode_workflow-2_v0.1.76.zip"
