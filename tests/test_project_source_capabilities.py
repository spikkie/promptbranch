...
class _AuthReadinessFakePage:
    ...
    async def evaluate(self, script, *args):
        return "Verify you are human Enable JavaScript and cookies to continue Cloudflare Turnstile"
...
def test_auth_readiness_snapshot_classifies_cloudflare_auth_block(browser_client: ChatGPTBrowserClient) -> None:
    page = _AuthReadinessFakePage()

    snapshot = asyncio.run(browser_client._auth_readiness_snapshot(page, label="unit-test"))

    assert snapshot["schema"] == "promptbranch.auth_readiness_snapshot"
    assert snapshot["challenge_detected"] is True
    assert snapshot["api_auth_error_detected"] is True
    assert snapshot["cloudflare_turnstile_detected"] is True
    assert browser_client._auth_readiness_is_blocked(snapshot) is True


def test_open_project_sources_tab_reports_auth_challenge_when_tab_missing(browser_client: ChatGPTBrowserClient) -> None:
    from promptbranch_browser_auth.exceptions import BotChallengeError

    page = _AuthReadinessFakePage()

    async def fake_wait_for_visible_locator(*_args, **_kwargs):
        return None

    browser_client._wait_for_visible_locator = fake_wait_for_visible_locator  # type: ignore[method-assign]

    with pytest.raises(BotChallengeError) as exc_info:
        asyncio.run(browser_client._open_project_sources_tab(page))

    payload = exc_info.value.to_payload()
    assert payload["status"] == "auth_challenge_blocking_before_project_sources"
    ...
    assert payload["auth_readiness"]["api_auth_error_detected"] is True
