...
def test_parser_accepts_phase3_src_sync_and_artifact_commands() -> None:
    ...
    artifact_mvp_status = parser.parse_args([
        "artifact", "mvp-status",
        "chatgpt_claudecode_workflow_v0.0.245.8.zip",
        "--version", "v0.0.245.8",
        "--repo-path", "/tmp/repo",
        "--json",
    ])
    ...
    assert artifact_mvp_status.artifact == "chatgpt_claudecode_workflow_v0.0.245.8.zip"
    assert artifact_mvp_status.version == "v0.0.245.8"
...
def test_parser_accepts_release_doctor_command() -> None:
    ...
    assert args.version == "v0.0.245.8"
