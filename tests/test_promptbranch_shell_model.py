...
def test_mutation_result_verified_contract_is_json_ready() -> None:
    result = mutation_result(
        ...
        verified=True,
        state_updated=True,
        artifact=ArtifactRef(source_ref="repo.zip", source_version="v0.0.133"),
    )
...
def test_state_store_tracks_workspace_task_and_artifact_independently(tmp_path) -> None:
    ...
    store.remember_artifact(
        ...
        artifact_ref="chatgpt_claudecode_workflow_v0.0.133.zip",
        artifact_version="v0.0.133",
        source_ref="chatgpt_claudecode_workflow_v0.0.133.zip",
        source_version="v0.0.133",
    )
    ...
    assert snapshot["artifact"]["artifact_version"] == "v0.0.133"
    assert snapshot["artifact"]["source_ref"] == "chatgpt_claudecode_workflow_v0.0.133.zip"

    on_disk = json.loads((tmp_path / ".promptbranch_state.json").read_text(encoding="utf-8"))
    assert on_disk["schema_version"] == 2
    assert on_disk["current"]["artifact_ref"] == "chatgpt_claudecode_workflow_v0.0.133.zip"
