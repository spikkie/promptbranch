...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["accepted_current_version"] == "v0.1.128.2"
    assert data["accepted_current_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.128.2.zip"
    assert data["active_candidate_version"] == "v0.1.128.2.5"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.128.2.5.zip"
    assert data["active_candidate_transport_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.128.2.5.zip"
    assert data["next_normal_version"] == "v0.1.129"
    assert data["active_slice"] == "v0.1.128.2.5 — Authoritative baseline auto-resolution repair"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["active_candidate_version"] == "v0.1.128.2.5"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.128.2.5.zip"
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    (repo / "VERSION").write_text("v0.1.128.2\n", encoding="utf-8")
    status = repo / "docs" / "project" / "status.md"
    text = status.read_text(encoding="utf-8")
    status.write_text(text.replace("accepted_current_artifact: chatgpt_claudecode_workflow-2_v0.1.128.2.zip", "accepted_current_artifact: chatgpt_claudecode_workflow-2_v0.1.79.zip"), encoding="utf-8")
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.128.2.zip"
...
def test_post_adoption_control_projection_advances_next_normal_without_changing_version_file(tmp_path: Path) -> None:
    ...
    result = synchronize_project_control_after_adoption(
        ...
        artifact_filename="chatgpt_claudecode_workflow-2_v0.1.128.2.5.zip",
        sha256="a" * 64,
    )
    assert result["ok"] is True, result
    state = json.loads((repo / "docs/project/plan-state.json").read_text())
    assert state["accepted_current_version"] == "v0.1.128.2.5"
