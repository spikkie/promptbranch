...
def test_release_status_has_allowed_table_and_current_baseline() -> None:
    ...
    assert "v0.1.99.1" in text
    assert "accepted/current" in text
    assert "chatgpt_claudecode_workflow-2_v0.1.99.1.zip" in text
    assert "v0.1.100" in text
    assert "First controlled read-only validation command execution" in text
...
def test_status_has_next_safe_action_and_accepted_baseline() -> None:
    ...
    assert "chatgpt_claudecode_workflow-2_v0.1.99.1.zip" in text
    assert "chatgpt_claudecode_workflow-2_v0.1.100.zip" in text
    assert "First controlled read-only validation command execution" in text
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    status.write_text(text.replace("chatgpt_claudecode_workflow-2_v0.1.99.1.zip", "chatgpt_claudecode_workflow-2_v0.1.79.zip", 1), encoding="utf-8")
...
def test_architecture_and_slice_horizon_are_documented() -> None:
    ...
    for version in ["v0.1.100", "v0.1.101", "v0.1.102", "v0.1.103", "v0.1.104"]:
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.99.1.zip"
    assert payload["next_normal_version"] == "v0.1.100"
    assert payload["next_normal_slice"] == "First controlled read-only validation command execution"
    assert payload["next_slice_after_acceptance_version"] == "v0.1.101"
    assert payload["next_slice_after_acceptance"] == "Read-only command result diagnosis and blocked/failed classification"
