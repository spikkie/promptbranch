...
def test_release_status_has_allowed_table_and_current_baseline() -> None:
    ...
    assert "chatgpt_claudecode_workflow-2_v0.1.121.1.zip" in text
    assert "chatgpt_claudecode_workflow-2_v0.1.121.1.zip" in text
...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["accepted_current_version"] == "v0.1.121.1"
    assert data["accepted_current_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.121.1.zip"
    assert data["active_candidate_version"] == "v0.1.122"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.122.zip"
    assert data["active_candidate_transport_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.122.zip"
    assert data["next_normal_version"] == "v0.1.122"
    assert data["active_slice"] == "v0.1.122 — Canonical MVP proof cycle 1"
    assert data["next_planned_version_after_acceptance"] == "v0.1.123"
    assert data["next_planned_slice_after_acceptance"] == "v0.1.123 — Canonical MVP proof cycle 2 and final MVP verdict"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["accepted_current_version"] == "v0.1.121.1"
    assert payload["active_candidate_version"] == "v0.1.122"
    assert payload["next_normal_slice"] == "v0.1.122 — Canonical MVP proof cycle 1"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.122.zip"
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    (repo / "VERSION").write_text("v0.1.122\n", encoding="utf-8")
    status = repo / "docs" / "project" / "status.md"
    text = status.read_text(encoding="utf-8")
    status.write_text(text.replace("accepted/current artifact: chatgpt_claudecode_workflow-2_v0.1.121.1.zip", "accepted/current artifact: chatgpt_claudecode_workflow-2_v0.1.79.zip"), encoding="utf-8")
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.121.1.zip"
    assert payload["next_normal_version"] == "v0.1.122"
    assert payload["next_normal_slice"] == "v0.1.122 — Canonical MVP proof cycle 1"
    assert payload["next_slice_after_acceptance_version"] == "v0.1.123"
    assert payload["next_slice_after_acceptance"] == "v0.1.123 — Canonical MVP proof cycle 2 and final MVP verdict"
