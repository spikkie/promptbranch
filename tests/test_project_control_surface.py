...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    status.write_text(text.replace("accepted/current baseline with adoption evidence: chatgpt_claudecode_workflow-2_v0.1.109.zip", "accepted/current baseline with adoption evidence: chatgpt_claudecode_workflow-2_v0.1.79.zip", 1), encoding="utf-8")
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.109.zip"
    assert payload["next_normal_version"] == "v0.1.109.1"
    assert payload["next_normal_slice"] == "v0.1.109.1 — Behavioral surface inventory and runtime authority resolver alignment"
