...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["active_candidate_version"] == "v0.1.104.8"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.104.8.zip"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["active_candidate_version"] == "v0.1.104.8"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.104.8.zip"
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    (repo / "VERSION").write_text("v0.1.104.8\n", encoding="utf-8")
