...
def test_release_status_has_allowed_table_and_current_baseline() -> None:
    ...
    assert "chatgpt_claudecode_workflow-2_v0.1.111.5.zip" in text
...
def test_status_has_next_safe_action_and_accepted_baseline() -> None:
    ...
    assert "chatgpt_claudecode_workflow-2_v0.1.111.5.zip" in text
    assert "chatgpt_claudecode_workflow-2_v0.1.111.5.1.zip" in text
    assert "standard browser profile default" in text
...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["accepted_current_version"] == "v0.1.111.5"
    assert data["accepted_current_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.111.5.zip"
    assert data["active_candidate_version"] == "v0.1.111.5.1"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.111.5.1.zip"
    assert data["active_candidate_transport_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.111.5.1.zip"
    assert data["next_normal_version"] == "v0.1.112"
    assert data["active_slice"] == "v0.1.111.5.1 — Empty-step-safe ETA progress and stable range countdown"
    assert data["next_planned_version_after_acceptance"] == "v0.1.112"
    assert data["next_planned_slice_after_acceptance"] == "v0.1.112 — PBAI-001 declaration and structural validation"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["accepted_current_version"] == "v0.1.111.5"
    assert payload["active_candidate_version"] == "v0.1.111.5.1"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.111.5.1.zip"
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    status.write_text(text.replace("accepted/current artifact: chatgpt_claudecode_workflow-2_v0.1.111.5.zip", "accepted/current artifact: chatgpt_claudecode_workflow-2_v0.1.79.zip", 1), encoding="utf-8")
...
def test_architecture_and_slice_horizon_are_documented() -> None:
    ...
    for version in ["v0.1.109", "v0.1.109.1", "v0.1.109.1.1", "v0.1.111", "v0.1.111.2", "v0.1.111.3", "v0.1.111.4", "v0.1.111.5", "v0.1.111.5.1", "v0.1.112"]:
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.111.5.zip"
    assert payload["next_normal_version"] == "v0.1.112"
    assert payload["next_normal_slice"] == "v0.1.112 — PBAI-001 declaration and structural validation"
    assert payload["next_slice_after_acceptance_version"] == "v0.1.112"
    assert payload["next_slice_after_acceptance"] == "v0.1.112 — PBAI-001 declaration and structural validation"
