...
def test_release_status_has_allowed_table_and_current_baseline() -> None:
    ...
    assert "chatgpt_claudecode_workflow-2_v0.1.123.2.2.zip" in text
...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["accepted_current_version"] == "v0.1.123.2.2"
    assert data["accepted_current_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.123.2.2.zip"
    assert data["active_candidate_version"] == "v0.1.123.2.3"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.123.2.3.zip"
    assert data["active_candidate_transport_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.123.2.3.zip"
    assert data["next_normal_version"] == "v0.1.124"
    assert data["active_slice"] == "v0.1.123.2.3 — Operation-scoped response guardrails and nested timeout budget repair"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["accepted_current_version"] == "v0.1.123.2.2"
    assert payload["active_candidate_version"] == "v0.1.123.2.3"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.123.2.3.zip"
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    (repo / "VERSION").write_text("v0.1.123.2.3\n", encoding="utf-8")
    status = repo / "docs" / "project" / "status.md"
    text = status.read_text(encoding="utf-8")
    status.write_text(text.replace("accepted/current artifact: chatgpt_claudecode_workflow-2_v0.1.123.2.2.zip", "accepted/current artifact: chatgpt_claudecode_workflow-2_v0.1.79.zip"), encoding="utf-8")
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.123.2.2.zip"
