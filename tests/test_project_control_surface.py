...
def test_release_status_has_allowed_table_and_current_baseline() -> None:
    ...
    assert "v0.1.103.5" in text
    assert "Docker parity true keep-open browser session mode" in text
...
def test_status_has_next_safe_action_and_accepted_baseline() -> None:
    ...
    assert "chatgpt_claudecode_workflow-2_v0.1.102.zip" in text
    assert "chatgpt_claudecode_workflow-2_v0.1.103.5.zip" in text
    assert "Docker parity true keep-open browser session mode" in text
...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["active_candidate_version"] == "v0.1.103.5"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.103.5.zip"
    assert data["next_normal_version"] == "v0.1.103.5"
    assert data["active_slice"] == "Docker parity true keep-open browser session mode"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["active_candidate_version"] == "v0.1.103.5"
    assert payload["next_normal_slice"] == "Docker parity true keep-open browser session mode"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.103.5.zip"
...
def test_architecture_and_slice_horizon_are_documented() -> None:
    ...
    for version in ["v0.1.103.5", "v0.1.104", "v0.1.105", "v0.1.106", "v0.1.107"]:
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["next_normal_version"] == "v0.1.103.5"
    assert payload["next_normal_slice"] == "Docker parity true keep-open browser session mode"
