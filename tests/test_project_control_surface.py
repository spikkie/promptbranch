...
def test_release_status_has_allowed_table_and_current_baseline() -> None:
    ...
    assert "Sandbox mutation verification and rollback evidence gate" in text
...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["accepted_current_version"] == "v0.1.103"
    assert data["accepted_current_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.103.zip"
    ...
    assert data["active_slice"] == "Sandbox mutation verification and rollback evidence gate"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["accepted_current_version"] == "v0.1.103"
    assert payload["active_candidate_version"] == "v0.1.104.3"
    assert payload["next_normal_slice"] == "Sandbox mutation verification and rollback evidence gate"
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    (repo / "VERSION").write_text("v0.1.104.3\n", encoding="utf-8")
    status = repo / "docs" / "project" / "status.md"
    text = status.read_text(encoding="utf-8")
    status.write_text(text.replace("chatgpt_claudecode_workflow-2_v0.1.103.zip", "chatgpt_claudecode_workflow-2_v0.1.79.zip", 1), encoding="utf-8")
...
def test_architecture_and_slice_horizon_are_documented() -> None:
    ...
    for version in ["v0.1.104", "v0.1.105", "v0.1.106", "v0.1.107", "v0.1.108"]:
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.103.zip"
