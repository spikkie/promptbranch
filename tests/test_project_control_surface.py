...
def test_release_status_has_allowed_table_and_current_baseline() -> None:
    ...
    assert "v0.1.100.3" in text
    assert "accepted/current" in text
    assert "chatgpt_claudecode_workflow-2_v0.1.100.3.zip" in text
    assert "v0.1.101" in text
    assert "Read-only command result diagnosis and blocked/failed classification" in text
...
def test_status_has_next_safe_action_and_accepted_baseline() -> None:
    ...
    assert "chatgpt_claudecode_workflow-2_v0.1.100.3.zip" in text
    assert "chatgpt_claudecode_workflow-2_v0.1.101.zip" in text
    assert "Read-only command result diagnosis and blocked/failed classification" in text
...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["accepted_current_version"] == "v0.1.100.3"
    assert data["accepted_current_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.100.3.zip"
    assert data["active_candidate_version"] == "v0.1.101"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.101.zip"
    assert data["next_normal_version"] == "v0.1.101"
    assert data["active_slice"] == "Read-only command result diagnosis and blocked/failed classification"
    assert data["next_planned_version_after_acceptance"] == "v0.1.102"
    assert data["next_planned_slice_after_acceptance"] == "Correction-plan generation without file mutation"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["accepted_current_version"] == "v0.1.100.3"
    assert payload["active_candidate_version"] == "v0.1.101"
    assert payload["next_normal_slice"] == "Read-only command result diagnosis and blocked/failed classification"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.101.zip"
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    (repo / "VERSION").write_text("v0.1.100\n", encoding="utf-8")
    status = repo / "docs" / "project" / "status.md"
    text = status.read_text(encoding="utf-8")
    status.write_text(text.replace("chatgpt_claudecode_workflow-2_v0.1.100.3.zip", "chatgpt_claudecode_workflow-2_v0.1.79.zip", 1), encoding="utf-8")
...
def test_architecture_and_slice_horizon_are_documented() -> None:
    ...
    for version in ["v0.1.101", "v0.1.102", "v0.1.103", "v0.1.104", "v0.1.105"]:
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.100.3.zip"
    assert payload["next_normal_version"] == "v0.1.101"
    assert payload["next_normal_slice"] == "Read-only command result diagnosis and blocked/failed classification"
    assert payload["next_slice_after_acceptance_version"] == "v0.1.102"
    assert payload["next_slice_after_acceptance"] == "Correction-plan generation without file mutation"
