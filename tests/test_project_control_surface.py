...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["accepted_current_version"] == "v0.1.128.1.1.1.1.1"
    assert data["accepted_current_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.128.1.1.1.1.1.zip"
    assert data["active_candidate_version"] == "v0.1.128.2"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.128.2.zip"
    assert data["active_candidate_transport_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.128.2.zip"
    assert data["next_normal_version"] == "v0.1.128.2"
    assert data["active_slice"] == "v0.1.128.2 — Promptbranch learning and skills completeness"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["accepted_current_version"] == "v0.1.128.1.1.1.1.1"
    assert payload["active_candidate_version"] == "v0.1.128.2"
    assert payload["next_normal_slice"] == "v0.1.128.2 — Promptbranch learning and skills completeness"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.128.2.zip"
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    (repo / "VERSION").write_text("v0.1.128.1.1.1.1.1\n", encoding="utf-8")
    status = repo / "docs" / "project" / "status.md"
    text = status.read_text(encoding="utf-8")
    status.write_text(text.replace("accepted_current_artifact: chatgpt_claudecode_workflow-2_v0.1.128.1.1.1.1.1.zip", "accepted_current_artifact: chatgpt_claudecode_workflow-2_v0.1.79.zip"), encoding="utf-8")
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.128.1.1.1.1.1.zip"
    assert payload["next_normal_version"] == "v0.1.128.2"
    assert payload["next_normal_slice"] == "v0.1.128.2 — Promptbranch learning and skills completeness"
...
def test_project_control_surface_validator_rejects_stale_active_candidate_version(tmp_path: Path) -> None:
    ...
    payload = validate_project_control_surface(repo)
    assert payload["ok"] is False
    assert any("VERSION 'v0.1.103.10.98' must match active candidate 'v0.1.103.10.72'" in error for error in payload["errors"])
...
def test_project_control_surface_fails_closed_on_authoritative_current_drift(tmp_path: Path, monkeypatch) -> None:
    import hashlib
    import shutil
    import zipfile
    ...
    record = ArtifactRecord(
        path=str(source), filename=filename, kind="adopted_release", version=version,
        repo_path=str(repo), sha256=digest, size_bytes=source.stat().st_size, file_count=1,
        created_at=utc_now(), repo_id=identity.repo_id,
    )
    registry.add(record)

    plan_state = repo / "docs/project/plan-state.json"
    data = json.loads(plan_state.read_text())
    data["accepted_current_sha256"] = digest
    plan_state.write_text(json.dumps(data, indent=2) + "\n")
    payload = validate_project_control_surface(repo)
    assert payload["ok"] is True, payload["errors"]
    assert payload["authoritative_current_checked"] is True
    assert payload["control_projection_matches_authoritative_current"] is True

    data["accepted_current_version"] = "v0.1.128"
    data["accepted_current_artifact"] = f"{identity.repo_id}_v0.1.128.zip"
    plan_state.write_text(json.dumps(data, indent=2) + "\n")
    payload = validate_project_control_surface(repo)
    assert payload["ok"] is False
    assert payload["control_projection_matches_authoritative_current"] is False
    assert any("differs from authoritative current" in error for error in payload["errors"])


def test_post_adoption_control_projection_advances_next_normal_without_changing_version_file(tmp_path: Path) -> None:
    import shutil
    from promptbranch_project_control import synchronize_project_control_after_adoption
    ...
    result = synchronize_project_control_after_adoption(
        repo,
        version="v0.1.128.2",
        artifact_filename="chatgpt_claudecode_workflow-2_v0.1.128.2.zip",
        sha256="a" * 64,
    )
    assert result["ok"] is True, result
    state = json.loads((repo / "docs/project/plan-state.json").read_text())
    assert state["accepted_current_version"] == "v0.1.128.2"
...
def test_post_adoption_projection_is_complete_and_immediately_valid(tmp_path: Path) -> None:
    ...
    assert "docs/project/migration.md" in CONTROL_PROJECTION_PATHS

    payload = validate_project_control_surface(repo)
    assert payload["ok"] is True, payload["errors"]
    assert payload["accepted_current_version"] == "v0.1.128.2"
    ...
    for rel in (
        ...
        assert "v0.1.128.1.1.1.1.1" in text
