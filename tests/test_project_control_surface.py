...
def test_release_status_has_allowed_table_and_current_baseline() -> None:
    ...
    assert "v0.1.103.10.116" in text
    assert "accepted/current" in text
    assert "chatgpt_claudecode_workflow-2_v0.1.103.10.116.zip" in text
...
def test_status_has_next_safe_action_and_accepted_baseline() -> None:
    ...
    assert "chatgpt_claudecode_workflow-2_v0.1.103.10.116.zip" in text
    assert "chatgpt_claudecode_workflow-2_v0.1.104.4.zip" in text
    assert "standard browser profile default" in text
...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["accepted_current_version"] == "v0.1.103.10.116"
    assert data["accepted_current_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.103.10.116.zip"
    assert data["active_candidate_version"] == "v0.1.104.4"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.104.4.zip"
    assert data["active_candidate_transport_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.104.4.zip"
    assert data["next_normal_version"] == "v0.1.104.4"
    assert data["active_slice"] == "v0.1.104.4 — parse-independent visual reply completion and bounded envelope recovery"
    assert data["next_planned_version_after_acceptance"] == "v0.1.105"
    assert data["next_planned_slice_after_acceptance"] == "Sandbox correction promotion readiness check"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["accepted_current_version"] == "v0.1.103.10.116"
    assert payload["active_candidate_version"] == "v0.1.104.4"
    assert payload["next_normal_slice"] == "v0.1.104.4 — parse-independent visual reply completion and bounded envelope recovery"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.104.4.zip"
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    status.write_text(text.replace("chatgpt_claudecode_workflow-2_v0.1.103.10.116.zip", "chatgpt_claudecode_workflow-2_v0.1.79.zip", 1), encoding="utf-8")
...
def test_architecture_and_slice_horizon_are_documented() -> None:
    ...
    for version in ["v0.1.103.10.67", "v0.1.103.10.69", "v0.1.103.10.70", "v0.1.103.10.71", "v0.1.103.10.90", "v0.1.103.10.91", "v0.1.103.10.92", "v0.1.103.10.93", "v0.1.103.10.94", "v0.1.103.10.95", "v0.1.103.10.96", "v0.1.103.10.97", "v0.1.103.10.98", "v0.1.103.10.99", "v0.1.103.10.100", "v0.1.103.10.101", "v0.1.103.10.102", "v0.1.103.10.103", "v0.1.103.10.104", "v0.1.103.10.105", "v0.1.103.10.106", "v0.1.103.10.107", "v0.1.103.10.109", "v0.1.103.10.110", "v0.1.103.10.111", "v0.1.103.10.112", "v0.1.103.10.113", "v0.1.103.10.114", "v0.1.103.10.115", "v0.1.103.10.116", "v0.1.104", "v0.1.104.1", "v0.1.104.2", "v0.1.104.4", "v0.1.105", "v0.1.106", "v0.1.107"]:
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.103.10.116.zip"
    assert payload["next_normal_version"] == "v0.1.104.4"
    assert payload["next_normal_slice"] == "v0.1.104.4 — parse-independent visual reply completion and bounded envelope recovery"
    assert payload["next_slice_after_acceptance_version"] == "v0.1.105"
    assert payload["next_slice_after_acceptance"] == "Sandbox correction promotion readiness check"
