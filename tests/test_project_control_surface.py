...
def test_status_has_next_safe_action_and_accepted_baseline() -> None:
    ...
    assert "chatgpt_claudecode_workflow-2_v0.1.109.1.1.zip" in text
    assert "chatgpt_claudecode_workflow-2_v0.1.110.zip" in text
    assert "standard browser profile default" in text
...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["active_candidate_version"] == "v0.1.110"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.110.zip"
    assert data["active_candidate_transport_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.110.zip"
    assert data["next_normal_version"] == "v0.1.110"
    assert data["active_slice"] == "v0.1.110 — Tracked backlog and architecture ticket intake"
    assert data["next_planned_version_after_acceptance"] == "v0.1.111"
    assert data["next_planned_slice_after_acceptance"] == "v0.1.111 — Global release lifecycle contract and read-only planner"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["active_candidate_version"] == "v0.1.110"
    assert payload["next_normal_slice"] == "v0.1.110 — Tracked backlog and architecture ticket intake"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.110.zip"
...
def test_architecture_and_slice_horizon_are_documented() -> None:
    ...
    for version in ["v0.1.108.1", "v0.1.109", "v0.1.109.1", "v0.1.109.1.1", "v0.1.110", "v0.1.111"]:
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["next_normal_version"] == "v0.1.110"
    assert payload["next_normal_slice"] == "v0.1.110 — Tracked backlog and architecture ticket intake"
    assert payload["next_slice_after_acceptance_version"] == "v0.1.111"
    assert payload["next_slice_after_acceptance"] == "v0.1.111 — Global release lifecycle contract and read-only planner"
