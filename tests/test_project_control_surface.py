...
def test_plan_state_is_machine_readable_next_slice_authority() -> None:
    ...
    assert data["accepted_current_version"] == "v0.1.112"
    assert data["accepted_current_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.112.zip"
    assert data["active_candidate_version"] == "v0.1.113"
    assert data["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.113.zip"
    assert data["active_candidate_transport_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.113.zip"
    assert data["next_normal_version"] == "v0.1.113"
    assert data["active_slice"] == "v0.1.113 — PBAI-001 registry validation and reference resolution"
    assert data["next_planned_version_after_acceptance"] == "v0.1.114"
    assert data["next_planned_slice_after_acceptance"] == "v0.1.114 — PBAI-001 executable validation and SkillRun evidence"
...
def test_project_control_surface_validator_passes_current_repo() -> None:
    ...
    assert payload["accepted_current_version"] == "v0.1.112"
    assert payload["active_candidate_version"] == "v0.1.113"
    assert payload["next_normal_slice"] == "v0.1.113 — PBAI-001 registry validation and reference resolution"
...
def test_project_control_surface_cli_emits_json() -> None:
    ...
    assert payload["active_candidate_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.113.zip"
...
def test_project_control_surface_validator_rejects_drifted_status(tmp_path: Path) -> None:
    ...
    status.write_text(text.replace("accepted/current artifact: chatgpt_claudecode_workflow-2_v0.1.112.zip", "accepted/current artifact: chatgpt_claudecode_workflow-2_v0.1.79.zip", 1), encoding="utf-8")
...
def test_architecture_and_slice_horizon_are_documented() -> None:
    ...
    for version in ["v0.1.111", "v0.1.111.2", "v0.1.111.3", "v0.1.111.4", "v0.1.111.5", "v0.1.111.5.2", "v0.1.112", "v0.1.113", "v0.1.114"]:
...
def test_project_next_slice_payload_is_derived_from_validated_control_surface() -> None:
    ...
    assert payload["ok"] is True, payload.get("errors")
    assert payload["baseline_artifact"] == "chatgpt_claudecode_workflow-2_v0.1.112.zip"
    assert payload["next_normal_version"] == "v0.1.113"
    assert payload["next_normal_slice"] == "v0.1.113 — PBAI-001 registry validation and reference resolution"
    assert payload["next_slice_after_acceptance_version"] == "v0.1.114"
    assert payload["next_slice_after_acceptance"] == "v0.1.114 — PBAI-001 executable validation and SkillRun evidence"
