...
def test_library_backend_protocol_diagnostic_install_mode_skips_release_mutation_and_adoption() -> None:
    text = (ROOT / "install.sh").read_text(encoding="utf-8")
    assert "--diagnostic-library-backend-protocol-reupload" in text
    diagnostic = text.split("if [[ ${diagnostic_library_backend_protocol_reupload} -eq 1 ]]; then", 1)[1].split("fi", 1)[0]
    assert "--skip-commit" in diagnostic
    assert "--skip-source-add" in diagnostic
    assert "--skip-tests" in diagnostic
    assert "--adopt-after-validation" not in diagnostic
    assert "pb-library-backend-protocol-reupload-diagnostic.sh" in diagnostic
