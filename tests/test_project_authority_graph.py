...
def test_version_projection_drift_is_detected(tmp_path: Path) -> None:
    ...
    pyproject.write_text(pyproject.read_text().replace('version = "0.1.123.2.4"', 'version = "9.9.9"'), encoding="utf-8")
...
def test_plan_state_markdown_projection_drift_is_detected(tmp_path: Path) -> None:
    ...
    status.write_text(status.read_text().replace("chatgpt_claudecode_workflow-2_v0.1.123.2.3.zip", "drifted.zip"), encoding="utf-8")
