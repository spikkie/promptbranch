...
def test_build_test_status_selects_latest_valid_full_suite_log(tmp_path):
    ...
    new = tmp_path / "pb_test.full.v0.0.190.log"
    old.write_text(json.dumps(_suite_payload(), indent=2), encoding="utf-8")
    payload = _suite_payload()
    payload["agent"]["version"] = "v0.0.190"
    ...
    assert status["selected_log"]["path"].endswith("pb_test.full.v0.0.190.log")
    assert status["suite"]["version"] == "v0.0.190"
...
def test_build_test_status_ignores_report_and_status_derivative_logs(tmp_path):
    ...
    suite_log = tmp_path / "pb_test.full.v0.0.190.log"
    report_log = tmp_path / "pb_test.full.v0.0.190.log.report"
    status_log = tmp_path / "pb_test.full.v0.0.190.log.status"
    ...
    assert "pb_test.full.v0.0.190.log.status" not in candidate_names
    assert "pb_test.full.v0.0.190.log.report" not in candidate_names
    assert "pb_test.full.v0.0.190.log" in candidate_names
...
def test_build_test_status_does_not_hide_newest_invalid_full_suite_log(tmp_path):
    ...
    assert status["latest_log"]["path"].endswith("pb_test.full.v0.0.190.log")
