...
def find_candidate_scrollables(page) -> list[dict[str, Any]]:
    ...
    () => {
      ...
      for (const el of nodes) {
        ...
        if (!isScrollable && !["auto", "scroll"].includes(overflowY) && !["auto", "scroll"].includes(overflowX)) {
          continue;
        }
...
def choose_scroll_target(page):
    ...
    () => {
      ...
      for (const el of nodes) {
        ...
        const score =
          (containsProjects ? 100000 : 0)
          + (el.scrollHeight - el.clientHeight)
          + rect.height
          - Math.abs(rect.left - 550);
        if (score > bestScore) {
          bestScore = score;
          best = el;
        }
...
def main() -> int:
    ...
    ap.add_argument("--url", default="https://chatgpt.com/", help="Start URL. Use a project URL if you want.")
    ...
    ap.add_argument("--browser-channel", default="chromium", help="Playwright browser channel.")
    ...
    with sync_playwright() as pw:
        ...
        print(f"Using profile dir: {args.profile_dir}")
        print(f"Writing debug artifacts to: {out_dir}")
        ...
        save_json(out_dir / "scroll_target_before_scroll.json", choose_scroll_target(page))
