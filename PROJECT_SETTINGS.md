...
## v0.1.121 resumable release-set recovery policy

Release-set recovery begins with read-only reconciliation. The original release-set ID, original plan SHA-256, checkpoint bytes, exact pre-rollout artifact and Project Source identities, and authoritative current registry identities form one immutable recovery decision. Reconciliation may classify only exact target, exact previous, missing, or ambiguous current state and must emit a canonical reconciliation SHA-256 without mutation.

Mutation through `pb release set resume` requires exact confirmation of the release-set ID, original plan SHA-256, latest reconciliation SHA-256, mandatory rollback-on-failure, and the complete per-repository release-pipeline authorization. Already verified target repositories must not be replayed. Interrupted rollback may touch only repositories still at the exact target identity and must proceed in reverse dependency order. Operator-repaired rollback may be finalized only after every repository is authoritatively restored to its exact pre-rollout artifact and Project Source identity. Missing or ambiguous identity is never inferred or overridden automatically.
