...
| `release_pipeline` | yes | Guard read-only planning, explicit mutation dependencies, same-run Git ordering, source-evidence-bound adoption, exact assigned-source current verification, and PBAI inventory. | `python3 -m pytest -q tests/test_promptbranch_release_contract.py tests/test_promptbranch_release_pipeline.py` |
