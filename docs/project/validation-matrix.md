...
| `application_architecture_structural` | yes | Validate the tracked PBAI-001 declaration, external-application pilot bootstrap contract, separate-repository/read-only authority, migration, all ten architecture layers, delegation, and proof-level reporting. | `python3 -m pytest -q tests/test_promptbranch_application_architecture.py tests/test_promptbranch_application_migration.py tests/test_promptbranch_application_pilot.py` |
...
| `version_surface` | yes | Validate sole `VERSION` authority across Python, dynamic package metadata, Docker build contract, exact-ZIP Docker-gate logic, and change-only-`VERSION` propagation. | `python3 -m pytest -q tests/test_promptbranch_version.py tests/test_version_authority_end_to_end.py tests/test_exact_zip_docker_build_gate.py` |
...
| `release_state_machine` | yes | Guard durable SHA-bound release attempts, all independently verifiable states, legal transitions, idempotent resume, automatic candidate registration, explicit mutation gates, and final convergence. | `python3 -m pytest -q tests/test_release_state_machine.py` |
| `release_pipeline` | yes | Guard read-only planning/import, incremental checkpoints, explicit mutation dependencies, Git/source evidence reuse, partial-failure recovery, source-evidence-bound adoption, exact current verification, PBAI inventory, and canonical MVP proof-cycle evaluation. | `python3 -m pytest -q tests/test_promptbranch_release_contract.py tests/test_promptbranch_release_pipeline.py tests/test_promptbranch_mvp_proof.py` |
...
| `exact_final_zip_docker_build` | yes (artifact stage) | Build the service image from the exact final ZIP and verify version, artifact SHA-256, source fingerprint, and attempt-id labels before broader lifecycle execution. | `python3 scripts/verify-exact-zip-docker-build.py --package-zip <exact-final.zip> --json` |
