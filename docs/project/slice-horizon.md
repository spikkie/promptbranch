...
|---|---|---|---|---|
| v0.1.103.8 | Docker parity Cloudflare challenge settle loop | active | Wire passive auth readiness into the actual Promptbranch browser client used by the service | Project Source mutation, deployment, artifact adoption |
...
## v0.1.103.8 active repair horizon

`v0.1.103.8` is the active Docker parity repair horizon. It focuses only on Cloudflare challenge settling with one held `/app/profile` browser session, same-session polling, and bounded evidence export. Project Source mutation remains out of scope.
