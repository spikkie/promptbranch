...
| v0.1.89 | normal | Live validation timing visibility and shortest-path click audit | candidate | focused browser-action audit tests, test-report timing/action aggregation tests, version/control-surface tests, compileall, shell syntax, Artifact Guardian, ZIP hygiene | Adds DOD-109 focused; builds from accepted/current v0.1.88.1 | pending adoption evidence |
