...
| v0.1.67 | normal | Project MVP / DoD / Plan control surface migration | candidate / build input | focused `tests/test_project_control_surface.py`; full tests not run here | DOD-001..DOD-007 moved to done; DOD-011 remains open | pending adoption evidence |
| v0.1.68 | normal | Project Sources add performance and transactional diagnostics | candidate | targeted source-add tests, control-surface test, compileall, ZIP hygiene; full tests not run | DOD-012 added/done after focused validation; DOD-009 done after ZIP hygiene | pending adoption evidence |
