...
## Migration note — v0.1.103.10.39 release-control clears auth bootstrap held session explicitly
...
## v0.1.103.10.39 migration

No manual state migration is required when `.pb_profile/browser/default` exists and is authenticated. During `--run-all-tests`, release-control can create `.pb_profile_local_debug` from that Docker standard browser profile. Operators who intentionally want a separate live seed may still provide `PROMPTBRANCH_RUN_ALL_LIVE_PROFILE_SEED_DIR` and/or `PROMPTBRANCH_RUN_ALL_LIVE_PROFILE_SEED_SOURCE_DIR`.
