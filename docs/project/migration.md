...
## v0.1.122 to v0.1.122.1 migration note

No registry, Project Source, artifact, or repository migration is performed. `v0.1.122` remains accepted/current until the repair is adopted.

Existing MVP proof evidence generated by `v0.1.122` must be re-evaluated under the repaired contract. Evidence is reusable only when all of the following are present and exact:

- real artifact-intake download and verification;
- canonical candidate SHA-256;
- adoption artifact version, filename, and SHA-256;
- project-level `repos.<repo-id>` current version, filename, and SHA-256;
- strict 10/10 GO and visual ZIP transport evidence.

The finalizer writes a preflight result before any continuation Ask. Failed preflight produces no continuation request/run evidence. A repair release cannot count toward the two-consecutive-normal-release MVP rule; the next normal cycles are `v0.1.123` and `v0.1.124`.


## v0.1.122.1 to v0.1.123 migration note

No registry, Project Source, release-set manifest, or application-state migration is required. `v0.1.122.1` remains accepted/current until strict validation adopts `v0.1.123`.
