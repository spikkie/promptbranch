...
Accepted/current baseline: `v0.1.129.1` (`chatgpt_claudecode_workflow-2_v0.1.129.1.zip`), SHA-256 `3edecfc3c052ea2895d45cdbc63a3aabf7693be229b32a41404026a73c4f3c68`.

Active candidate: `v0.1.130` (`chatgpt_claudecode_workflow-2_v0.1.130.zip`).

## Current candidate scope

- add `pb application change plan`, `apply`, and `rollback`;
- permit only tracked, bounded file writes in a separate established application repository;
- require explicit `--execute` and an exact `--authorize-change <change-id>` human authorization before mutation;
- persist exact pre-state byte snapshots and metadata in Promptbranch operator state before the first target mutation;
- verify every post-write SHA-256 and automatically restore exact pre-state on any partial failure;
- refuse explicit rollback when post-apply target bytes have drifted;
- execute no Git command, Git publication, Project Source mutation, deployment, artifact adoption, or application-test command.

`v0.1.131 — Application test, diagnosis, and bounded correction loop` is planned after acceptance. Rolling horizon authority remains `docs/project/plan-state.json`.
...
> Live control projection after adoption: accepted/current `v0.1.130` (`chatgpt_claudecode_workflow-2_v0.1.130.zip`), SHA-256 `347098567709cdf44ddeab938bef90b24fb59d0ba8d3662f934a771edce7da1c`. Active next normal slice is `v0.1.131 — Application test, diagnosis, and bounded correction loop` with artifact `chatgpt_claudecode_workflow-2_v0.1.131.zip`. Planned after acceptance is `v0.1.132 — External application candidate and acceptance lifecycle` (`v0.1.132`) with artifact `chatgpt_claudecode_workflow-2_v0.1.132.zip`.
...
```text
accepted_current_version: v0.1.130
accepted_current_artifact: chatgpt_claudecode_workflow-2_v0.1.130.zip
accepted_current_sha256: 347098567709cdf44ddeab938bef90b24fb59d0ba8d3662f934a771edce7da1c
active_candidate_version: v0.1.131
active_candidate_artifact: chatgpt_claudecode_workflow-2_v0.1.131.zip
active_candidate_base_version: v0.1.130
next_normal_version: v0.1.131
next_normal_slice: v0.1.131 — Application test, diagnosis, and bounded correction loop
```

## Current interpretation

accepted/current artifact: chatgpt_claudecode_workflow-2_v0.1.128.2.6.1.1.1.zip
accepted/current version: v0.1.128.2.6.1.1.1
...
## v0.1.128.2.1 timeout-recovery status

Active repair candidate `chatgpt_claudecode_workflow-2_v0.1.128.2.1.zip` preserves the complete v0.1.128.2 learning/skills release. Release smoke asks no longer require a second operator lifecycle invocation after supported transient service/assistant-response timeouts: the original canonical test command performs bounded observe-then-retry recovery internally and records every attempt. Recovery remains fail-closed for auth/challenge, 429/cooldown, exact-route mismatch, permission, and ambiguous submit-causality states. Construction proof, deterministic exact ZIP/Guardian, and one fresh live lifecycle are required before accepted/current may move. `v0.1.129` remains blocked until that live proof closes; `v0.1.130` remains planned after `.129`.

## v0.1.128.2.2 accepted-runtime reconciliation status

Active repair candidate `chatgpt_claudecode_workflow-2_v0.1.128.2.2.zip` removes the remaining manual prerequisite before `RUNTIME_PREPARED`: an absent, unhealthy, or wrong-version port-8000 service is reconstructed automatically from the exact adopted/current artifact in PB's registry. Candidate bytes are never used as baseline authority. Reconciliation proceeds only after repo/version/SHA/ZIP/VERSION proof and is independently health-checked before isolated candidate preparation. Authority ambiguity or invalid adopted bytes still fail closed. `v0.1.129` remains blocked until this exact repair reaches FINAL_VERIFIED/current.
