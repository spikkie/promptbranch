...
| DOD-274 | Artifact state uses one authoritative project registry; explicit repository identity and configured membership are mandatory; missing, invalid, unreadable, stale repo-local, or noncanonical registry state fails closed without migration | in_progress | `v0.1.103.10.105` clean-break registry, artifact, control-surface, and packaged validation | v0.1.103.10.105 |
