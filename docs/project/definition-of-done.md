...
| DOD-122 | Run-all progress writer emits valid JSON without embedded-Python newline syntax errors, and `browser_scheduler_source_lifecycle` timeout diagnostics expose active/completed/failed/timed-out pytest nodeids without changing required validation semantics | focused_candidate | `chatgpt_claudecode_workflow_release_control.sh`, `promptbranch_test_suite.py`, focused shell/test-suite regressions, `docs/repair-v0.1.91.10.md` | v0.1.91.10 |
