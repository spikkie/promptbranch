...
The docs-site guard now verifies that the source tree documents local preview and build commands, keeps generated `site/` output forbidden from committed source, and reports `docs_site.build_readiness.ok=true` from `pb release docs-status --version v0.1.65 --json`.
